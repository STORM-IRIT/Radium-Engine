...
TriangleMeshBSH::TriangleMeshBSH( std::vector<Vector3d> const& vertices,
...
Vector3d const& TriangleMeshBSH::entityPosition( unsigned int i ) const {
...
struct TBSHCoordAccessor {
    ...
    inline Cit operator()( Pit it ) const { return ( *vertices )[*it].data(); }
    std::set<unsigned int> const* indices;
    std::vector<Vector3d> const* vertices;
};

void TriangleMeshBSH::computeHull( unsigned int b, unsigned int n, BoundingSphere& hull ) const {
...
TriangleMeshBBH::TriangleMeshBBH( std::vector<Vector3d> const& vertices,
                                  std::vector<std::array<unsigned int, 3>> const& faces ) :
    super( faces.size() ),
    m_faces( faces ),
    m_vertices( vertices ),
...
Vector3d const& TriangleMeshBBH::entityPosition( unsigned int i ) const {
    return m_tri_centers[i];
}

void TriangleMeshBBH::computeHull( unsigned int b, unsigned int n, AlignedBox3d& hull ) const {
    for ( auto i = 0u; i < n; ++i )
    {
        auto const& f = m_faces[m_lst[b + i]];
        for ( auto v : f )
...
PointCloudBSH::PointCloudBSH() : super( 0 ) {}

PointCloudBSH::PointCloudBSH( std::vector<Vector3d> const& vertices ) :
    super( vertices.size() ),
    m_vertices( &vertices ) {}

Vector3d const& PointCloudBSH::entityPosition( unsigned int i ) const {
    return ( *m_vertices )[i];
}
...
struct PCBSHCoordAccessor {
    ...
    using Cit = double const*;
    inline Cit operator()( Pit it ) const { return ( *vertices )[( *lst )[it]].data(); }
    std::vector<Vector3d> const* vertices;
