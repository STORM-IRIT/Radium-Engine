...
MeshDistance::MeshDistance( TriangleMesh const& mesh, bool precompute_normals ) :
    ...
    m_cache.resize(
        max_threads,
        FunctionValueCache( [&]( Vector3d const& xi ) { return signedDistance( xi ); }, 10000u ) );
    m_ucache.resize(
        max_threads,
        FunctionValueCache( [&]( Vector3d const& xi ) { return distance( xi ); }, 10000u ) );
    ...
    if ( m_precomputed_normals )
    {
        ...
        std::transform( m_mesh.faces().begin(), m_mesh.faces().end(), m_face_normals.begin(),
                        [&]( std::array<unsigned int, 3> const& face ) {
                            ...
                            auto alpha =
                                Vector3d{std::acos( e1.dot( -e3 ) ), std::acos( e2.dot( -e1 ) ),
                                         std::acos( e3.dot( -e2 ) )};
...
// Thread-safe.
double MeshDistance::distance( Vector3d const& x, Vector3d* nearest_point,
    ...
    if ( f < m_mesh.nFaces() )
    {
        auto t = std::array<Vector3d const*, 3>{&m_mesh.vertex( m_mesh.faceVertex( f, 0 ) ),
...
bool MeshDistance::predicate( unsigned int node_index, TriangleMeshBSH const& bsh,
                              Vector3d const& x, double& dist_candidate ) const {
...
void MeshDistance::callback( unsigned int node_index, TriangleMeshBSH const& bsh, Vector3d const& x,
...
double MeshDistance::signedDistance( Vector3d const& x ) const {
    unsigned int nf;
...
double MeshDistance::signedDistanceCached( Vector3d const& x ) const {
    return m_cache[omp_get_thread_num()]( x );
}

double MeshDistance::unsignedDistance( Vector3d const& x ) const {
    return distance( x );
}

double MeshDistance::unsignedDistanceCached( Vector3d const& x ) const {
    return m_ucache[omp_get_thread_num()]( x );
}

Vector3d MeshDistance::face_normal( unsigned int f ) const {
    if ( m_precomputed_normals )
...
Vector3d MeshDistance::edge_normal( Halfedge const& h ) const {
    auto o = m_mesh.opposite( h );

    if ( m_precomputed_normals )
    {
        if ( o.isBoundary() )
            return m_face_normals[h.face()];
        return m_face_normals[h.face()] + m_face_normals[o.face()];
    }

    if ( o.isBoundary() )
        return face_normal( h.face() );
    return face_normal( h.face() ) + face_normal( o.face() );
}

Vector3d MeshDistance::vertex_normal( unsigned int v ) const {
    if ( m_precomputed_normals )
