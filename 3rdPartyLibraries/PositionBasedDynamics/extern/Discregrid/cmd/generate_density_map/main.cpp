...
std::istream& operator>>( std::istream& is, AlignedBox3d& data ) {
    is >> data.min()[0] >> data.min()[1] >> data.min()[2] >> data.max()[0] >> data.max()[1] >>
        data.max()[2];
    return is;
}
...
int main( int argc, char* argv[] ) {
    cxxopts::Options options(
        argv[0], "Generates a signed distance field from a closed two-manifold triangle mesh." );
    options.positional_help( "[input OBJ file]" );

    options.add_options()( "h,help", "Prints this help text" )(
        "r,rest_density", "Rest density rho0 of the fluid",
        cxxopts::value<double>()->default_value( "1000.0" ) )( "i,invert", "Invert field" )(
        "s,smoothing_length", "Kernel smoothing length",
        cxxopts::value<double>()->default_value( "0.1" ) )(
        "o,output", "Ouput file in cdf format",
        cxxopts::value<std::string>()->default_value( "" ) )(
        "no-reduction", "Disables discarding of cells for sparse layout." )(
        "input", "Discrete grid file containing input SDF in field 0",
    ...
    if ( options.count( "help" ) )
    {
        ...
        std::cout << std::endl
                  << std::endl
                  << "Example: GenerateSDF -r \"50 50 50\" dragon.obj" << std::endl;
        exit( 0 );
    }
    if ( !options.count( "input" ) )
    {
        ...
        std::cout << std::endl
                  << std::endl
                  << "Example: GenerateDensityMap -r \"50 50 50\" field.cdf" << std::endl;
        exit( 1 );
    }
    ...
    if ( options["no-reduction"].count() == 0u )
    {
        ...
        sdf->reduceField( 1u,
                          [&]( auto const&, double v ) { return 0.0 <= v && v <= 3.0 * rho0; } );
        std::cout << "DONE" << std::endl;
    }
