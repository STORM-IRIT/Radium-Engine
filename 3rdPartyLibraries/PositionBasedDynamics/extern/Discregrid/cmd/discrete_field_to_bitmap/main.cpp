...
std::array<unsigned char, 3u> doubleToGreenBlueInverse( double v ) {
    if ( v >= 0.0 )
    {
        return {
            {0u,
             static_cast<unsigned char>( std::min( std::max( 255.0 * ( 1.0 - v ), 0.0 ), 255.0 ) ),
             0u}};
    }
    return {
        {0u, 0u,
...
int main( int argc, char* argv[] ) {
    ...
    options.add_options()( "h,help", "Prints this help text" )(
        "f,field_id", "ID in which the SDF to export is stored.",
        cxxopts::value<unsigned int>()->default_value( "0" ) )(
        "s,samples", "Number of samples in width direction",
        cxxopts::value<unsigned int>()->default_value( "1024" ) )(
        "p,plane", "Plane in which the image slice is extracted",
        cxxopts::value<std::string>()->default_value( "xy" ) )(
        "d,depth",
        "Relative depth value between -1 and 1 in direction of the axis orthogonal to the plane",
        cxxopts::value<double>()->default_value( "0" ) )(
        "o,output", "Output (in bmp format)", cxxopts::value<std::string>()->default_value( "" ) )(
        "c,colormap",
        "Color map options: redsequential (rs), green blue inverse diverging (gb) (suitable for "
        "visualiztion of signed distance fields)",
        cxxopts::value<std::string>()->default_value( "gb" ) )(
    ...
    auto depth = options["d"].as<double>();
    auto const& domain = sdf->domain();
    auto diag = domain.diagonal().eval();

    auto plane = options["p"].as<std::string>();
    if ( plane.length() != 2 && plane[0] != plane[1] )
    {
        std::cerr << "ERROR: Invalid option for plane provided. Should be one of the following "
                     "options: xy, xz, yz, yx"
                  << std::endl;
        exit( 1 );
    }
    ...
    auto xsamples = options["s"].as<unsigned int>();
    auto ysamples = static_cast<unsigned int>(
        std::round( diag( dir( 1 ) ) / diag( dir( 0 ) ) * static_cast<double>( xsamples ) ) );
    ...
    auto field_id = options["f"].as<unsigned int>();
...
#pragma omp parallel for
    ...
    auto out_file = options["o"].as<std::string>();
    if ( out_file == "" )
    {
        out_file = filename;
    ...
    std::cout << "Export BMP...";
    std::transform( data.begin(), data.end(), data.begin(), [&max_v, &min_v]( double v ) {
        return v >= 0.0 ? v / std::abs( max_v ) : v / std::abs( min_v );
    } );

    auto pixels = std::vector<std::array<unsigned char, 3u>>( data.size() );

    auto cm = options["c"].as<std::string>();
    if ( cm != "gb" && cm != "rs" )
    {
    ...
    std::cout << "\tdomain         = " << domain.min().transpose() << ", "
              << domain.max().transpose() << std::endl;
