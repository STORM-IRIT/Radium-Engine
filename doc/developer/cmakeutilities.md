...
if(NewComponent_FOUND and NOT TARGET NewComponent)
    ...
    if(NOT Core_FOUND) # The dependency on Core was not explicitely requested by the user
        ...
        if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/../Core/RadiumCoreConfig.cmake")
            ...
            set(Radium_NOTFOUND_MESSAGE "Radium::NewComponent : dependency Core not found")
...
if(Configure_NewComponent)
    ...
    if(MSVC OR MSVC_IDE OR MINGW)
        add_imported_dir(FROM ExternalDependency::Target TO RADIUM_EXTERNAL_DLLS_LOCATION)
    endif()
...
foreach(_comp ${<packageName>_FIND_COMPONENTS})
  ...
  if (${${_comp}_FOUND} EQUAL -1)
    ...
    set(<packageName>_NOTFOUND_MESSAGE "Unsupported <packageName> component: ${_comp}")
  else()
...
if (<secondLib>_FOUND)
    ...
    if (NOT <firstLib>_FOUND)
        ...
        set(<packageName>_FOUND False)
        set(<packageName>_NOTFOUND_MESSAGE "Component <secondLib> requires the component <firstLib>")
