...
void NodeGeometry::recalculateSize() const {
    ...
    if ( auto w = _dataModel->embeddedWidget() ) { _width += w->width(); }
...
QPointF NodeGeometry::widgetPosition() const {
    if ( auto w = _dataModel->embeddedWidget() ) {
        if ( w->sizePolicy().verticalPolicy() & QSizePolicy::ExpandFlag ) {
            // If the widget wants to use as much vertical space as possible, place it immediately
            // after the caption.
            ...
            if ( _dataModel->validationState() != NodeValidationState::Valid ) {
                return QPointF(
                    ...
                    ( captionHeight() + _height - validationHeight() - _spacing - w->height() ) /
                        2.0 );
            }
...
QPointF NodeGeometry::calculateNodePositionBetweenNodePorts( PortIndex targetPortIndex,
                                                             ...
                                                             Node* sourceNode,
                                                             Node& newNode ) {
    // Calculating the nodes position in the scene. It'll be positioned half way between the two
    // ports that it "connects". The first line calculates the halfway point between the ports (node
    // position + port position on the node for both nodes averaged). The second line offsets this
    // coordinate with the size of the new node, so that the new nodes center falls on the
    // originally calculated coordinate, instead of it's upper left corner.
...
unsigned int NodeGeometry::portWidth( PortType portType ) const {
    ...
    for ( auto i = 0ul; i < _dataModel->nPorts( portType ); ++i ) {
        ...
        else { name = _dataModel->dataType( portType, i ).name; }
