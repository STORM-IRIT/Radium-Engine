...
bool NodeConnectionInteraction::canConnect( PortIndex& portIndex, TypeConverter& converter ) const {
    ...
    if ( requiredPort == PortType::None ) { return false; }
    ...
    if ( portIndex == INVALID ) { return false; }
...
bool NodeConnectionInteraction::tryConnect() const {
    ...
    if ( !canConnect( portIndex, converter ) ) { return false; }

    // 1.5) If the connection is possible but a type conversion is needed,
    //      assign a convertor to connection
    if ( converter ) { _connection->setTypeConverter( converter ); }
...
PortIndex
NodeConnectionInteraction::nodePortIndexUnderScenePoint( PortType portType,
