...
class NodeConnectionInteraction
{
  public:
    ...
    /// 4) Connection type equals node port type, or there is a registered type conversion that can
    /// translate between the two
    bool canConnect( PortIndex& portIndex, TypeConverter& converter ) const;

    /// 1)   Check conditions from 'canConnect'
    /// 1.5) If the connection is possible but a type conversion is needed, add a converter node to
    /// the scene, and connect it properly 2)   Assign node to required port in Connection 3) Assign
    /// Connection to empty port in NodeState 4)   Adjust Connection geometry 5)   Poke model to
