...
Connection::~Connection() {
    ...
    if ( _inNode ) { _inNode->nodeGraphicsObject().update(); }

    if ( _outNode ) { _outNode->nodeGraphicsObject().update(); }
}
...
void Connection::setNodeToPort( Node& node, PortType portType, PortIndex portIndex ) {
    ...
    updated( *this );
    if ( complete() && wasIncomplete ) { connectionCompleted( *this ); }
}
...
NodeDataType Connection::dataType( PortType portType ) const {
    if ( _inNode && _outNode ) {
        auto const& model =
            ( portType == PortType::In ) ? _inNode->nodeDataModel() : _outNode->nodeDataModel();
...
void Connection::propagateData( std::shared_ptr<NodeData> nodeData ) const {
    if ( _inNode ) {
        if ( _converter ) { nodeData = _converter( nodeData ); }
