        ...
        for ( unsigned int i = 0; i < n; ++i ) {
            ...
            bool canConnect =
                ( state.getEntries( portType )[i].empty() ||
                  ( portType == PortType::Out && model->portOutConnectionPolicy( i ) ==
            ...
            else { painter->setBrush( nodeStyle.ConnectionPointColor ); }
...
void NodePainter::drawEntryLabels( QPainter* painter,
    ...
    for ( PortType portType : { PortType::Out, PortType::In } ) {
        ...
        for ( size_t i = 0; i < n; ++i ) {
            ...
            else { s = model->dataType( portType, i ).name; }
...
void NodePainter::drawValidationRect( QPainter* painter,
    ...
    if ( modelValidationState != NodeValidationState::Valid ) {
        ...
        else { painter->setBrush( nodeStyle.WarningColor ); }
