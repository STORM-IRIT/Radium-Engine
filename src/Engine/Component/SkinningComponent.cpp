...
void SkinningComponent::initialize() {
    ...
    if ( hasSkel && hasMesh && hasRefPose )
    {
        ...
        // get other data
        m_refData.m_skeleton = compMsg->get<Skeleton>( getEntity(), m_skelName );
        createWeightMatrix();
        m_refData.m_refPose = compMsg->get<RefPose>( getEntity(), m_skelName );
        ...
        m_weightTechnique->setParametersProvider( mat );
        // assign texture
...
void SkinningComponent::endSkinning() {
    if ( m_frameData.m_doSkinning )
    {
        ...
        uniformNormal( vertices, m_refData.m_referenceMesh.m_indices, m_duplicatesMap, normals );
...
void SkinningComponent::setupSkinningType( SkinningType type ) {
    ...
    case STBS_LBS:
    {
        ...
        {
            Ra::Core::Animation::computeSTBS_weights(
                m_refData.m_referenceMesh.vertices(), m_refData.m_skeleton, m_weightSTBS );
        }
    }
    } // end of switch.
}
