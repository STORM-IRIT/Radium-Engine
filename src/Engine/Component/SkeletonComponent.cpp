...
void SkeletonComponent::setSkeleton( const Ra::Core::Animation::Skeleton& skel ) {
    m_skel    = skel;
    m_refPose = skel.getPose( HandleArray::SpaceType::MODEL );
    setupSkeletonDisplay();
}

void SkeletonComponent::update( Scalar dt ) {
    if ( dt != 0.0 )
    {
    ...
    if ( dt > 0 && !m_animations.empty() )
    {
        ...
        m_skel.setPose( currentPose, HandleArray::SpaceType::LOCAL );
    }
...
void SkeletonComponent::handleSkeletonLoading( const Ra::Core::Asset::HandleData* data ) {
    ...
    m_refPose = m_skel.getPose( HandleArray::SpaceType::MODEL );
...
void SkeletonComponent::toggleSlowMotion( const bool status ) {
    m_slowMo = status;
}

void SkeletonComponent::setAnimation( const uint i ) {
...
Scalar SkeletonComponent::getTime() const {
    return m_animationTime;
}

Scalar SkeletonComponent::getDuration() const {
    if ( m_animations.empty() ) { return Scalar( 0 ); }
    return m_animations[m_animationID].getDuration();
}

uint SkeletonComponent::getMaxFrame() const {
    if ( m_animations.empty() ) { return 0; }
    return uint( std::round( getDuration() / m_dt[m_animationID] ) );
}

void SkeletonComponent::cacheFrame( const std::string& dir, int frame ) const {
...
bool SkeletonComponent::restoreFrame( const std::string& dir, int frame ) {
    ...
    { return false; } auto pose = m_skel.getPose( HandleArray::SpaceType::LOCAL );
