...
class RA_ENGINE_API Renderer {
  ...
  public:
    ...
    enum PickingMode {
        ...
        C_VERTEX,  ///< Pick all vertices of a mesh within a screen space circle.
        C_EDGE,    ///< Pick all edges of a mesh within a screen space circle.
        C_TRIANGLE ///< Pick all triangles of a mesh within a screen space circle.
    };
    ...
    /**
     * Picking result.
     * \note There is exactly one data Index triplet for each querried pixel (thus
  ...
  protected:
    ...
    /// The render queue for non xray-ed objects in the scene.
  ...
  private:
    ...
    /// The render queues for picking non xray-ed objects in the scene.
    std::array<std::vector<RenderObjectPtr>, 4> m_fancyRenderObjectsPicking;

    /// The render queues for picking xray-ed objects in the scene.
