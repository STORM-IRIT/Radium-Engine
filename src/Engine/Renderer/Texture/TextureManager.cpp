...
TextureData TextureManager::loadTexture( const std::string& filename ) {
    ...
    int n;
    unsigned char* data =
        stbi_load( filename.c_str(), &( texData.width ), &( texData.height ), &n, 0 );
...
// FIXME : for the moment, Texture name is equivalent to file name if the
//         texture is loaded by the manager.
