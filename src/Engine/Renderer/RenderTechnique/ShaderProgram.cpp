        ...
        if ( std::regex_search( l, match, reg ) )
        {
            ...
            } else
            {
                LOG( Core::Utils::logWARNING )
                    << "Cannot open included file " << match[1].str() << " at line" << nline
                    << " of file " << name << ". Ignored.";
                continue;
            }
            /*
             std::string inc;
             ...
             {
             LOG(Core::Utils::logWARNING) << "Cannot open included file " << file << " from " <<
             m_filename << ". Ignored."; continue;
             }
             */
        }
