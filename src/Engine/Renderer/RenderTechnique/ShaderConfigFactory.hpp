...
namespace Engine {
   ...
   * Usefull for the management of shader libraries where a shader configuration (source code, defines, properties)
   * are defined once and reuse multiple time.
   */
...
RA_ENGINE_API void addConfiguration( const ShaderConfiguration& config );
     ...
     * @note If the requested configuration is not in the factory, return a default shader configuration.
     */
