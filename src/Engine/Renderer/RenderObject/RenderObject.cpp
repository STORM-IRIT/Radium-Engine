...
namespace Ra {
    namespace Engine {
        ...
        //       const RenderData& rdata, const ShaderProgram* altShader )


    void updateAttribIndex(Mesh *m, const ShaderProgram *p){
        const Ra::Core::TriangleMesh &tm = m->getGeometry();
        for(const auto & attr : tm.attribManager().attribs()){

            int loc =  p->getProgramObject()->getAttributeLocation(attr->getName());
            if(loc != -1)
            {
                m->bindVao();
/// \todo                m->bindBuffer(attr->getName());
...
//                updateAttribIndex(getMesh().get(), shader);
