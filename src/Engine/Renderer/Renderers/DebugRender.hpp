...
class RA_ENGINE_API DebugRender {
  ...
  public:
    ...
    void addLine( const Core::Math::Vector3& from, const Core::Math::Vector3& to,
    ...
    void addPoints( const Core::Container::Vector3Array& p,
                    const Core::Container::Vector4Array& colors );
    void addMesh( const std::shared_ptr<Mesh>& mesh,
                  const Core::Math::Transform& transform = Core::Math::Transform::Identity() );

    // Shortcuts
    void addCross( const Core::Math::Vector3& position, Scalar size,
                   const Core::Math::Color& color );

    void addSphere( const Core::Math::Vector3& center, Scalar radius,
                    const Core::Math::Color& color );

    void addCircle( const Core::Math::Vector3& center, const Core::Math::Vector3& normal,
                    Scalar radius, const Core::Math::Color& color );

    void addFrame( const Core::Math::Transform& transform, Scalar size );

    void addTriangle( const Core::Math::Vector3& p0, const Core::Math::Vector3& p1,
  ...
  private:
    struct Line {
        Line( const Core::Math::Vector3& la, const Core::Math::Vector3& lb,
              const Core::Math::Color& lcol ) :
            a( la ),
            b( lb ),
            col( lcol ) {}
