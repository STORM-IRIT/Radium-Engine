    ...
    auto createProgram = []( const char* vertStr, const char* fragStr ) -> uint {
        ...
        if ( !compiled )
        {
            ...
            LOG( Core::Utils::logERROR )
                << "Vertex shader not compiled : " << std::string( log.data() ) << "\n"
                << vertStr;
        }
        ...
        if ( !compiled )
        {
            ...
            LOG( Core::Utils::logERROR )
                << "Fragment shader not compiled : " << std::string( log.data() ) << "\n"
                << fragStr;
        }
...
void DebugRender::render( const Core::Math::Matrix4& viewMatrix,
                          const Core::Math::Matrix4& projMatrix ) {
...
void DebugRender::renderMeshes( const Core::Math::Matrix4f& view,
                                const Core::Math::Matrix4f& proj ) {
    if ( m_meshes.empty() )
    {
        return;
    }
...
void DebugRender::addPoints( const Core::Container::Vector3Array& p,
                             const Core::Container::Vector4Array& c ) {
...
void DebugRender::addMesh( const std::shared_ptr<Mesh>& mesh,
                           const Core::Math::Transform& transform ) {
    m_meshes.push_back( {mesh, transform} );
}

void DebugRender::addCross( const Core::Math::Vector3& position, Scalar size,
                            const Core::Math::Color& color ) {
