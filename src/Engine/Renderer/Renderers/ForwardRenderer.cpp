...
void ForwardRenderer::renderInternal( const RenderData& renderData ) {
    ...
    const Core::Math::Colorf clearColor =
        Core::Math::FromChars<Core::Math::Colorf>( 42, 42, 42, 0 );
    const Core::Math::Colorf clearZeros = Core::Math::Black<Core::Math::Colorf>();
    const Core::Math::Colorf clearOnes =
        Core::Math::FromChars<Core::Math::Colorf>( 255, 255, 255, 255 );
    ...
    // LOG(Core::Utils::logDEBUG) << "Forward renderer has " << m_lightmanagers[0]->count() << "
    // lights."; forward renderer only use one light manager
