...
namespace Ra {
    namespace Engine {
        ...
        Renderer::Renderer()
        : m_width(0), m_height(0), m_shaderMgr(nullptr), m_displayedTexture(nullptr), m_renderQueuesUpToDate(false),
        m_quadMesh(nullptr), m_drawDebug(true), m_wireframe(false), m_postProcessEnabled(true)
        {
            GL_CHECK_ERROR;
        }
        ...
        uchar *Renderer::grabFrame(uint &w, uint &h) const
        {
            ...
            for (uint j = 0; j < tex->height(); ++j)
            {
                for (uint i = 0; i < tex->width(); ++i)
                {
                    ...
                    uint ou =
                    4 * ((tex->height() - 1 - j) * tex->width() + i); // Index in the final image (note the j flipping).
