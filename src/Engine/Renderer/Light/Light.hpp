...
class RA_ENGINE_API Light : public Component {
  public:
     ...
     * @todo for the moment, the attenuation is non physically coherent. make attenuation be quadratic by default
     */
    struct Attenuation {
        Scalar constant { 1 };
        Scalar linear { 0 };
...
public:
    ...
    /**
     * Extrac the set of parameters that must be given to a shader for rendering and lighting with this light.
     * @param params
     */
     ...
     * For the moment, this is not use (except by experimental plugins) but will part of the shader composition process
     * of radium v2
     * @return
     */
