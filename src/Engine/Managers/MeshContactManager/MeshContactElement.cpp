...
//                return false;
//            }
//            else if (! getProgressiveMeshLOD()->getProgressiveMesh()->isEcolPossible(he, d.m_p_result))
//            {
//                LOG(logINFO) << "Collapse not possible";
...
//            else
        ...
        void MeshContactElement::computePrimitives()
        {
            ...
            int nbVertex = getProgressiveMeshLOD()->getProgressiveMesh()->getTopologicalMesh()->n_vertices();
//            int nbVertex = getProgressiveMeshLOD()->getProgressiveMesh()->getDcel()->m_vertex.size();

            for (uint v = 0; v < nbVertex; v++)
            {
                std::vector<Ra::Core::TopologicalMesh::FaceHandle> adjFaces;
                Ra::Core::TopologicalMesh::VertexFaceIter vf_it = getProgressiveMeshLOD()->getProgressiveMesh()->getTopologicalMesh()->vf_iter(getProgressiveMeshLOD()->getProgressiveMesh()->getTopologicalMesh()->vertex_handle(v));
