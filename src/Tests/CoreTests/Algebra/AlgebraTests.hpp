...
class AlgebraTests : public Test {
    void run() override {
        ...
        for ( uint i = 0; i < 3; ++i )
        {
            ...
            Ra::Core::Math::Vector3 fl( std::floor( v.x() ), std::floor( v.y() ),
                                        std::floor( v.z() ) );
            Ra::Core::Math::Vector3 ce( std::ceil( v.x() ), std::ceil( v.y() ),
                                        std::ceil( v.z() ) );
        ...
        RA_UNIT_TEST(
            Ra::Core::Math::AngleAxis( qt ).axis().isApprox( Ra::Core::Math::Vector3::UnitZ() ),
            "Twist should be around z" );
        RA_UNIT_TEST(
            Ra::Core::Math::AngleAxis( qs ).axis().dot( Ra::Core::Math::Vector3::UnitZ() ) == 0,
