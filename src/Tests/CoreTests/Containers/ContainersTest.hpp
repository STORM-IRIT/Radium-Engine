...
namespace RaTests
{
    class IteratorsTests : public Test
    {
        void run() override
        {
            ...
            Container reverted1; reverted1.reserve(nbSamples);
            Container reverted2; reverted2.reserve(nbSamples);
            ...
            Container reverted4; reverted4.reserve(nbSamples);
            for(auto v : Ra::Core::reversed(ccontainer))
                reverted4.push_back(v);

            RA_UNIT_TEST(reverted1 == reverted4, "Const qualifier should not change the loop behavior");

            // check we can also write in the containers
            for(auto& v : Ra::Core::reversed(reverted2)) v++;
            for(auto& v : reverted1) v++;
