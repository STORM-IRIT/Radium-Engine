...
namespace RaTests
{
    class DistanceTests : public Test
    {
        void run() override
        {
            ...
            RA_UNIT_TEST( distPointToTri == (c - (0.5 * (a + b))).norm(), "Distance point to triangle not ok.");
            ...
            Scalar distLineToSeg = Ra::Core::DistanceQueries::lineToSegSq(lineOrigin, lineDirection, segCenter, segDirection, segExtent).sqrDistance;
            RA_UNIT_TEST( distLineToSeg == Ra::Core::DistanceQueries::pointToSegmentSq(a, c, b-c), "Distance line to segment not ok.");

            // Test line to triangle distance query
            Vector3 v[3] = {a, d, e};
            Scalar distLineToTri = Ra::Core::DistanceQueries::lineToTriSq(segCenter, segDirection, v).sqrDistance;
            RA_UNIT_TEST( distLineToTri == Ra::Core::DistanceQueries::pointToSegmentSq(a, c, b-c), "Distance line to triangle not ok.");
