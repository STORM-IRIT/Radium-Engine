...
class RayCastAabbTests : public Test {
    void run() override {
        Ra::Core::Math::Aabb ones( -Ra::Core::Math::Vector3::Ones(),
                                   Ra::Core::Math::Vector3::Ones() );
                    ...
                    for ( int q = -10; q < 11; q++ )
                    {
                        ...
                        // Fire a ray towards the box (some hit, some miss).
                        {
                            ...
                            if ( std::abs( p ) <= 5 && std::abs( q ) <= 5 )
                            {
                                ...
                                RA_UNIT_TEST( n.dot( sig * Ra::Core::Math::Vector3::Unit( i ) ) ==
                                                  1.f,
                                              "Wrong normal" );
                                RA_UNIT_TEST(
                        ...
                        {
                            Ra::Core::Math::Ray r = Ra::Core::Math::Ray(
                                Ra::Core::Math::Vector3::Zero(), dir.normalized() );
