...
void LoggableFrameTimerData::printAverageTimerData(el::base::type::ostream_t& os) const
{
    ...
    os << "\tAverage render time : " << sumRender/N << " µs (" << N*1000000.f/float(sumRender) << " updates per second)" << std::endl;
    os << "\tAverage tasks time  : " << sumTasks/N << " µs (" << N*1000000.f/float(sumTasks) << " updates per second)" << std::endl;
    os << "\tAverage frame time  : " << sumFrame/N << " µs (" << N*1000000.f/float(sumFrame) << " max fps)" << std::endl;
