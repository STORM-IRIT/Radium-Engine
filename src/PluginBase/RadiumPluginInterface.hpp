...
namespace Plugins {
/// The RadiumPluginInterface define the interface to be implemented by the
/// RadiumEngine's MainApplication Plugins.
class RadiumPluginInterface {
  public:
     ...
     * This method is supposed to create the system (\em new it)
    ...
    /**
     * @brief Tells whether the plugin wants to add a widget
    ...
    /**
     * @brief Tells whether the plugin wants to add a menu
    ...
    /**
     * @brief Tells whether the plugin wants to add actions
    ...
    /**
     * @brief Tells whether the plugin wants to add Renderers to the Engine.
     * @return True if the plugin wants to add Renderers.
     */
    virtual bool doAddRenderer() { return false; }

    /**
     * @brief Creates the Renderers to be added to the Engine.
     *
    ...
    /**
     * @brief Tells whether the plugin wants to add FileLoaders.
     * @returnTrue if the plugin wants to add FileLoaders.
     */
    virtual bool doAddFileLoader() { return false; }

    /**
     * @brief Creates the FileLoaders to be added.
     *
     * \warning Allocated FileLoaders are given to the application and
     * SHOULD not be destroyed by the plugin.
     */
    virtual void
    ...
    /**
     * @brief Tells whether the plugin wants to add OpenGL resources.
     * @return True if the plugin wants to add OpenGL resources.
     */
     ...
     * @note Plugins that want to manage OpenGL resources MUST ENSURE at the time
     * they create/acces these resources that the current openGLContext is the
     * one pass when initializing OpenGL here and stored in the PluginContext ...
     * @note When this function is called from the application, The active OpenGL
     ...
     * (recomended for plugins that manage OpenGL resources for rendering or interaction
     * with rendering) must implement this method and memorize the viewer of the
     * PluginContext. Each time they need to create or destroy an OpenGL resource,
     * they must activate this context (viewer->makeCurrent()) and realeas it
     * after usage (viewer->doneCurrent()).
     * @see Ra::Gui::Viewer.
     */
