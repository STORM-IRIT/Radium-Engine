...
/**
 * Return true if \p p0 and \p p1 are approximately equal.
 */
RA_CORE_API bool areEqual( const Pose& p0, const Pose& p1 );

/**
 * Return the pose interpolating \p a and \p b, with respect to interpolation parameter \p t.
 */
RA_CORE_API Pose interpolatePoses( const Pose& a, const Pose& b, const Scalar t );

/**
 * Return in \p interpolated the pose interpolating \p a and \p b, with respect to interpolation
 * parameter \p t.
 */
