    ...
    for ( Eigen::SparseVector<Scalar>::InnerIterator it1( v1w ); it1; ++it1 )
    {
        ...
        if ( W2j > 0 )
        {
            for ( Eigen::SparseVector<Scalar>::InnerIterator it2( v2w ); it2; ++it2 )
            {
                ...
                if ( j != k  && W1k > 0)
                {
                    const Scalar diff = std::exp( - Math::ipow<2>( (W1j * W2k) - (W1k * W2j) ) / (sigmaSq) );
...
void computeCoR( Skinning::RefData& dataInOut, Scalar sigma = 0.1f, Scalar weightEpsilon = 0.1f )
{
    ...
    do
    {
        ...
        for ( const auto& edge : dcel.m_fulledge )
        {
            ...
            Scalar weightDistance = (subdividedWeights.row( v1->idx ) - subdividedWeights.row( v2->idx )).norm();
        ...
        if ( !edgesToSplit.empty() )
        {
            ...
            Eigen::SparseMatrix<Scalar, Eigen::RowMajor> newWeights( startIndex + edgesToSplit.size(), numCols );
            ...
            for ( const auto& edge : edgesToSplit )
            {
                ...
                subdividedWeights.row( startIndex + i ) = 0.5f * (subdividedWeights.row(V1Idx) + subdividedWeights.row(V2Idx));
                ++i;
            }
    ...
    CORE_ASSERT( subdividedMesh.m_vertices.size() == subdividedWeights.rows(), "Weights and vertices don't match" );
    ...
    for ( uint i = 0; i < nVerts; ++i )
    {
        ...
        for ( uint t = 0; t < subdividedMesh.m_triangles.size(); ++t )
        {
            ...
            const Eigen::SparseVector<Scalar> triWeight = (subdividedWeights.row( tri[0] ) + subdividedWeights.row( tri[1] ) + subdividedWeights.row( tri[2] )) / 3.f;
...
void corSkinning( const Vector3Array& input, const Animation::Pose& pose, const Animation::WeightMatrix& weight, const Vector3Array& CoR, Vector3Array& output )
{
