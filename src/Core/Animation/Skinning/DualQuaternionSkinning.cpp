...
void computeDQ_naive( const Pose& pose, const WeightMatrix& weight, DQList& DQ ) {
    ...
    for (uint i = 0; i < weight.rows(); ++i)
    {
        ...
        for (uint j = 0; j < weight.cols(); ++j)
        {
            ...
            // Flip the dual quaternion sign according to the dot product with the first non-null dual quat
            // see Algorithm 2 in section 4.1 of the paper.
            Scalar sign = Ra::Core::Math::signNZ(  poseDQ[j].getQ0().dot(poseDQ[firstNonZero].getQ0()));
