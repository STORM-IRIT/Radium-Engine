...
/// \name Weight Operations
/// \{

/**
 * Return the WeightMatrix extracted from the MeshWeight vector,
 * for a handle with handle_size transforms.
 */
 ...
 *       weights( i, j ) >= 0 and L1-norm( weights.row( i ) ) > 0
 */
...
/**
 * Check each vertex has at least one skinning weight, i.e.\ no empty row in \p matrix.
 */
