...
/// Return in \p result the linear interpolation between \p v0 and \p v1, according to \p t.
inline void interpolate( const Core::Vector3& v0, const Core::Vector3& v1, const Scalar t,
                         Core::Vector3& result ) {
    result = ( ( ( 1.0 - t ) * v0 ) + ( t * v1 ) );
}

/// Return in \p result the spherical linear interpolation between \p q0 and \p q1, according to \p
/// t.
...
/// Return in \p result the linear interpolation between \p q0 and \p q1, according to \p t.
/// To do so, the rotational parts are interpolated through spherical linear interpolation while the
/// translational parts are interpolated through linear interpolation.
