...
class KeyFrame {
  public:
    ...
    /// Replace the \p i-th FRAME by \p frame.
    ...
    /// Remove all FRAMES and empty the AnimationTime.
    inline void reset() {
        m_time.setStart( 0.0 );
        m_time.setEnd( 0.0 );
        clear();
    ...
    /// Return true if *this and \p keyframe have either a different AnimationTime or different
    /// FRAMEs.
  ...
  protected:
    ...
    /// Returns the linear interpolation between \p F0 and \p F1, according to \p t.
