...
void check( const AdjacencyList& adj ) {
    ...
    CORE_ASSERT( ( adj.m_parent.size() == adj.m_child.size() ),
                 "Children list and parent list are incompatible." );
    ...
    for ( uint node = 0; node < adj.size(); ++node )
    {
        ...
        CORE_ASSERT( adj.isLeaf( node ) == ( adj.m_child.at( node ).size() == 0 ),
                     " Only leaves should have no children." );
    }
#endif
}
...
Container::VectorArray<Geometry::Edge> extractEdgeList( const AdjacencyList& adj,
...
void storeAdjacencyList( const AdjacencyList& adj, const std::string& name ) {
    ...
    {
        uint c;
        content = content + std::to_string( i ) + " " + std::to_string( adj.m_parent[i] ) + " " +
                  std::to_string( c = adj.m_child[i].size() );
        for ( uint j = 0; j < c; ++j )
        {
