...
template <uint D, uint K>
inline void Spline<D, K>::setCtrlPoints(
    const Core::Container::VectorArray<typename Spline<D, K>::Vector>& points ) {
    m_points = points;
...
template <uint D, uint K>
inline const Core::Container::VectorArray<typename Spline<D, K>::Vector>&
Spline<D, K>::getCtrlPoints() const {
    return m_points;
}
