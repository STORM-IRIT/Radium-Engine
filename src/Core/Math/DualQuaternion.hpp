...
class DualQuaternion {
  ...
  public:
    ...
    /// Return the non-dual part quaternion.
    inline const Quaternion& getQ0() const;

    /// Set the non-dual part quaternion.
    inline void setQ0( const Quaternion& q0 );

    /// Return the dual part quaternion.
    inline const Quaternion& getQe() const;

    /// Set the dual part quaternion.
    ...
    /// Return the corresponding rigid transform.
    /// \note Assume a unit dual quaternion.
    inline Transform getTransform() const;

    /// Normalize the quaternion with the dual-number norm (divides q0 and qe by q0's norm).
