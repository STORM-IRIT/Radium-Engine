...
bool RA_CORE_API vsAabb( const Ray& r, const Core::Aabb& aabb, Scalar& hitOut, Vector3& normalOut );

/// Intersects a ray with a sphere.
bool RA_CORE_API vsSphere( const Ray& r, const Core::Vector3& center, Scalar radius,
                           std::vector<Scalar>& hitsOut );

/// Intersect a ray with an infinite plane defined by point A and normal.
bool RA_CORE_API vsPlane( const Ray& r, const Core::Vector3 a, const Core::Vector3& normal,
                          std::vector<Scalar>& hitsOut );

/// Intersect  a ray with a cylinder with a and b as caps centers and given radius.
bool RA_CORE_API vsCylinder( const Ray& r, const Core::Vector3& a, const Core::Vector3& b,
                             Scalar radius, std::vector<Scalar>& hitsOut );

/// Intersect a ray with a triangle abc.
bool RA_CORE_API vsTriangle( const Ray& r, const Core::Vector3 a, const Core::Vector3& b,
                             const Core::Vector3& c, std::vector<Scalar>& hitsOut );

bool RA_CORE_API vsTriangleMesh( const Ray& r, const Geometry::TriangleMesh& mesh,
                                 std::vector<Scalar>& hitsOut,
