...
bool vsSphere( const Ray& r, const Vector3& center, Scalar radius, std::vector<Scalar>& hitsOut ) {
...
bool vsPlane( const Ray& r, const Vector3 a, const Vector3& normal, std::vector<Scalar>& hitsOut ) {
    CORE_ASSERT( r.direction().squaredNorm() > 0.f, "Invalid Ray" );
