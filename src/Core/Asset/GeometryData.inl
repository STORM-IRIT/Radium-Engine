...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        ...
        m_vertex[i] =
            Core::Math::Vector3( vertexList[i].template cast<Core::Math::Vector3::Scalar>() );
    }
}
...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        // unnecessary call to copy constructor and cast are removed at compile time
        m_edge[i] =
            Core::Math::Vector2ui( edgeList[i].template cast<Core::Math::Vector2ui::Scalar>() );
    }
}
...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        // unnecessary call to copy constructor and cast are removed at compile time
        m_faces[i] =
            Core::Math::VectorNui( faceList[i].template cast<Core::Math::VectorNui::Scalar>() );
    }
}
...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        // unnecessary call to copy constructor and cast are removed at compile time
        m_polyhedron[i] =
            Core::Math::VectorNui( polyList[i].template cast<Core::Math::VectorNui::Scalar>() );
    }
}
...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        // unnecessary call to copy constructor and cast are removed at compile time
        m_normal[i] =
            Core::Math::Vector3( normalList[i].template cast<Core::Math::Vector3::Scalar>() );
    }
}
...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        // unnecessary call to copy constructor and cast are removed at compile time
        m_tangent[i] =
            Core::Math::Vector3( tangentList[i].template cast<Core::Math::Vector3::Scalar>() );
    }
}
...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        // unnecessary call to copy constructor and cast are removed at compile time
        m_bitangent[i] =
            Core::Math::Vector3( bitangentList[i].template cast<Core::Math::Vector3::Scalar>() );
    }
}
...
#pragma omp parallel for
    for ( int i = 0; i < int( size ); ++i )
    {
        // unnecessary call to copy constructor and cast are removed at compile time
        m_texCoord[i] =
            Core::Math::Vector3( texCoordList[i].template cast<Core::Math::Vector3::Scalar>() );
    }
}
