...
class KeyScaling : public KeyFrame<Core::Math::Vector3> {
  public:
    /// CONSTRUCTOR
    KeyScaling( const AnimationTime& time = AnimationTime() ) :
  ...
  protected:
    /// TRANSFORMATION
    inline Core::Math::Vector3 defaultFrame() const override {
        return Core::Math::Vector3( 1.0, 1.0, 1.0 );
    }

    inline Core::Math::Vector3 interpolate( const Core::Math::Vector3& F0,
                                            const Core::Math::Vector3& F1,
