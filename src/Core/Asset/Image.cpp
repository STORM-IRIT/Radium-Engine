...
{
  public:
    ImageImpl( ImageSpec&& spec, const void* data, size_t len ) {
        OpenImageIO_v2_3::ImageSpec imgSpec;

        assert( data != nullptr );

        // copy spec into OIIO spec
        // the day we decide to use OpenImageIO in Radium, we have to delete this code and use
        // ImageSpec from OIIO future todo : delete this and use directly OIIO spec class
        {
        ...
        m_cpyData = new unsigned char[len];
        memcpy( m_cpyData, data, len );
        m_imgBuf = OpenImageIO_v2_3::ImageBuf( imgSpec, m_cpyData );
        assert( (void*)m_imgBuf.pixeladdr( 0, 0, 0 ) == m_cpyData );
        m_sizeData = getWidth() * getHeight() * getNChannels();
        assert( len == getSizeData() );
    }

    ImageImpl( ImageSpec&& spec, void* data ) {
        OpenImageIO_v2_3::ImageSpec imgSpec;
    ...
    void update( const void* data, size_t len ) {
        assert( getSizeData() == len );
        if ( m_cpyData == nullptr ) m_cpyData = new unsigned char[len];
        std::memcpy( m_cpyData, data, len );

        m_imgBuf = OpenImageIO_v2_3::ImageBuf( m_imgBuf.spec(), m_cpyData );
        //        m_imgBuf.reset( m_imgBuf.spec(), m_cpyData );
        assert( (void*)m_imgBuf.pixeladdr( 0, 0, 0 ) == m_cpyData );
    }

    void update( void* data ) {
        if ( data != m_imgBuf.pixeladdr( 0, 0, 0 ) ) {
            m_imgBuf = OpenImageIO_v2_3::ImageBuf( m_imgBuf.spec(), data );
            //            m_imgBuf.reset( m_imgBuf.spec(), data );
            assert( (void*)m_imgBuf.pixeladdr( 0, 0, 0 ) == data );
        }
    }
  ...
  private:
    size_t m_sizeData = 0;
    //    OpenImageIO_v2_3::ImageCache* m_imgCache = nullptr;
    OpenImageIO_v2_3::ImageBuf m_imgBuf;
    unsigned char* m_cpyData = nullptr;
};
...
class ImageImpl
{
  public:
    ImageImpl( ImageSpec&& spec, const void* data, size_t len ) {
        ...
        m_sizeData       = len;
        m_readWriteBuffs = new unsigned char[m_sizeData * 2]; // do this to benefit of cache
        m_data[0]        = m_readWriteBuffs;
        m_data[1]        = &m_readWriteBuffs[m_sizeData];
    ...
    void update( const void* data, size_t len ) {
        ...
        // no update if data is null
        // data can be null when user want to create an image without having the data
        ...
        std::memcpy( m_data[writeHead], data, len );
    ...
    void update( void* data ) {
        ...
        // copy user pointer image instead of copy all data
        const int writeHead = 1 - m_readHead;
        m_data[writeHead]   = data;
        m_readHead          = writeHead;
    }
    ...
    void resize( int width, int height, const void* data, size_t len ) {
        ...
        tmp.readWriteBuffs = new unsigned char[tmp.sizeData * 2]; // do this to benefit of cache
        tmp.data[0]        = tmp.readWriteBuffs;
        tmp.data[1]        = &tmp.readWriteBuffs[tmp.sizeData];

        const int writeHead = 1 - m_readHead;
        std::memcpy( tmp.data[writeHead], data, len );
        tmp.readHead = writeHead;

        void* dataToRemove = m_readWriteBuffs;
    ...
    void resize( int width, int height, void* data ) {
        ...
        tmp.sizeData = tmp.width * tmp.height * m_nChannels;

        tmp.readWriteBuffs = new unsigned char[tmp.sizeData * 2];
        tmp.data[0]        = tmp.readWriteBuffs;
        tmp.data[1]        = &tmp.readWriteBuffs[tmp.sizeData];

        const int writeHead = 1 - m_readHead;
        tmp.data[writeHead] = data;
        tmp.readHead        = writeHead;
  ...
  private:
    unsigned char* m_readWriteBuffs = nullptr;
...
Image::Image( ImageSpec spec, const void* data, size_t len ) :
    m_impl { new ImageImpl( std::move( spec ), data, len ) },
    m_age { 1 } // birth
{}

Image::Image( ImageSpec spec, void* data ) :
    m_impl { new ImageImpl( std::move( spec ), data ) },
    m_age { 1 } // birth
{}
...
void Image::update( const void* data, size_t len ) {
    m_impl->update( data, len );
    ++m_age;
}

void Image::update( void* data ) {
    m_impl->update( data );
    ++m_age;
}

void Image::resize( int width, int height, const void* data, size_t len ) {
    m_impl->resize( width, height, data, len );
    ++m_age;
}

void Image::resize( int width, int height, void* data ) {
    m_impl->resize( width, height, data );
    ++m_age;
}
