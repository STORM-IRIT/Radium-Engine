...
class RA_CORE_API LightData : public AssetData {
  ...
  public:
    ...
    /**
     * Copy constructor.
     * As a Light is a union like object, no default copy constructor could be
     * generated. This will take care of copying the good members of the union.
    ...
    /**
     * Extension of the AssetData interface so that we can modify an asset after
     * its creation. This has no impact on the coherence of the LightData object
     ...
     * \note The object on which this method is called is unconditionally promoted
     *       to ``DIRECTIONAL_LIGHT`` light, whatever it was before the call and
     *       only the directional light part of the union is consistent after the call.
     */
     ...
     * \note The object on which this method is called is unconditionally promoted
     *       to ``POINT_LIGHT`` light, whatever it was before the call and only
     *       the directional light part of the union is consistent after the call.
     */
    inline void setLight( const Core::Utils::Color& color,
     ...
     * A spot light is defined by its color, its position, the cone axis and
     * light distribution (constant inside \p inAngle, quadratically deacreasing
     * toward 0 from \p inAngle to \p outAngle) and its attenuation.
     * \note The object on which this method is called is unconditionally promoted
     *       to ``SPOT_LIGHT`` light, whatever it was before the call and only
     *       the directional light part of the union is consistent after the call.
     */
    inline void setLight( const Core::Utils::Color& color,
                          const Eigen::Matrix<Scalar, 3, 1>& position,
     ...
     * An area light, (isotropic with constant emision defined by its color) is
     * approximated by its center and the covariance matrices modelling the
     * spatial extent as well as the elliptical distribution of normals.
     * \note The object on which this method is called is unconditionally promoted
     *       to ``AREA_LIGHT`` light, whatever it was before the call and only
     *       the directional light part of the union is consistent after the call.
     */
  ...
  public:
    ...
    union {
        struct {
            Eigen::Matrix<Scalar, 3, 1> direction; ///< The light direction.
        } m_dirlight;                              ///< Directional Light parameters.
        struct {
            Eigen::Matrix<Scalar, 3, 1> position; ///< The light position.
            LightAttenuation attenuation;         ///< The light attenuation.
        } m_pointlight;                           ///< Point Light parameters.
        struct {
            Eigen::Matrix<Scalar, 3, 1> position;  ///< The light position.
            Eigen::Matrix<Scalar, 3, 1> direction; ///< The light direction.
            Scalar innerAngle;                     ///< The light inner angle.
            Scalar outerAngle;                     ///< The light outer angle.
            LightAttenuation attenuation;          ///< The light attenuation.
        } m_spotlight;                             ///< Spot Light parameters.
        struct {
            // TODO : this representation is usefull but might be improved
            Eigen::Matrix<Scalar, 3, 1> position;          ///< The light position.
            Eigen::Matrix<Scalar, 3, 3> spatialCovariance; ///< The light spatial covariance matrix.
            Eigen::Matrix<Scalar, 3, 3> normalCovariance;  ///< The light normal covariance matrix.
            LightAttenuation attenuation;                  ///< The light attenuation.
