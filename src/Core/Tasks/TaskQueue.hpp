...
namespace Core {
/**
 * This class allows tasks to be registered and then executed in parallel on
 * separate threads.
 * It maintains an internal pool of threads.
 * When instructed, it dispatches the tasks to the pooled threads.
 * Task are allowed to have dependencies.
 * A task will be executed only when all its dependencies are satisfied,
 * i.e.\ all dependant tasks are finished.
 * \note Most functions are not thread safe and must not be called when
 *       the task queue is running.
 */
class RA_CORE_API TaskQueue {
  ...
  public:
    /**
     * Initializes the thread pools with numThreads threads.
     */
    TaskQueue( uint numThreads );

    /**
     * Waits for all the threads and safely deletes them.
     */
    ~TaskQueue();
     ...
     * \p task must have been created with new and be initialized with its parameter.
     * \note The TaskQueue assumes the ownership of the Tasks.
     */
    TaskId registerTask( Task* task );

    /**
     * Add dependency between two tasks. The successor task will be executed only when all
     * its predecessors are completed.
     */
     ...
     * if the task is not present yet, the name being resolved when task starts.
     */
    ...
    /// \name Task queue operations
    /// \{

    /**
     * Launches the execution of all the threads in the task queue.
     * \note No more tasks should be added at this point.
     */
    void startTasks();
  ...
  private:
    ...
    /// List of pending dependencies to predecessor tasks.
    std::vector<std::pair<TaskId, std::string>> m_pendingDepsPre;

    /// List of pending dependencies to successor tasks.
