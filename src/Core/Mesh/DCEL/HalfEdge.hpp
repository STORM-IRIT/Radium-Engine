 ...
 * and points to the incident halfedge belonging to the neighboring face (the so called twin
 * halfedge).
 *
...
class[[deprecated]] RA_CORE_API HalfEdge : public IndexedObject {
  public:
    ...
    /// VERTEX
    inline const Vertex_ptr& V() const;      // Return the reference to the halfedge vertex
    ...
    /// NEXT
    inline const HalfEdge_ptr& Next() const;         // Return the reference to the next halfedge
    ...
    /// PREV
    inline const HalfEdge_ptr& Prev() const;         // Return the reference to the prev halfedge
    ...
    /// TWIN
    inline const HalfEdge_ptr& Twin() const;         // Return the reference to the twin halfedge
    ...
    /// HALFEDGE
    inline void setNeighborHE( const HalfEdge_ptr& next, const HalfEdge_ptr& prev,
                               const HalfEdge_ptr& twin ); // Set the neighboring halfedges

    /// FULLEDGE
    inline const FullEdge_ptr& FE() const;       // Return the reference to the fulledge
    ...
    /// FACE
    inline const Face_ptr& F() const;      // Return the reference to the face
