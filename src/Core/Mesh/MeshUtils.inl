...
inline void getTriangleVertices( const TriangleMesh& mesh, Index triIdx,
    ...
    const auto& tri = mesh.m_triangles[triIdx];
    for ( uint i = 0; i < 3; ++i )
    {
...
inline uint getLastVertex( const Vector3ui& t1, uint v1, uint v2 ) {
    CORE_ASSERT( t1[0] == v1 || t1[1] == v1 || t1[2] == v1, "Vertex 1 not in Vector3ui" );
    CORE_ASSERT( t1[0] == v2 || t1[1] == v2 || t1[2] == v2, "Vertex 2 not in Vector3ui" );
...
inline bool containsEdge( const Vector3ui& t1, uint v1, uint v2 ) {
