        ...
        namespace MeshUtils
        {
                ...
                for (uint n = 0; n < numSubdiv; ++n)
                {
                    ...
                    for (uint i = 0; i < result.m_triangles.size(); ++i)
                    {
                        ...
                        for (int v = 0; v < 3; ++v)
                        {
                            ...
                            result.vertices().push_back(0.5f * (triVertices[v] + triVertices[(v + 1) % 3]));
                        }
            ...
            TriangleMesh makeCylinder(const Vector3 &a, const Vector3 &b, Scalar radius, uint nFaces)
            {
                ...
                for (uint i = 0; i < nFaces; ++i)
                {
                    ...
                    result.vertices().push_back(a + radius* ( std::cos (theta) * xPlane + std::sin (theta) * yPlane ));
                    result.vertices().push_back(c + radius* ( std::cos (theta) * xPlane + std::sin (theta) * yPlane ));
                    result.vertices().push_back(b + radius* ( std::cos (theta) * xPlane + std::sin (theta) * yPlane ));
                }
            ...
            TriangleMesh makeCone(const Vector3 &base, const Vector3 &tip, Scalar radius, uint nFaces)
            {
                ...
                {
                    const Scalar theta = i * thetaInc;
                    result.vertices().push_back(base + radius * (std::cos(theta) * xPlane + std::sin(theta) * yPlane));
                }
