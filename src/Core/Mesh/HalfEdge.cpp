        ...
        for (TriangleIdx t = 0; t < mesh.m_triangles.size(); ++t)
        {
            ...
            for (uint i = 0; i < 3; ++i)
            {
                ...
                if (edgeFound == edgeToHalfEdges.end())
                {
                    ...
                    m_vertexToHalfEdge[vStart] = baseIdx;
                    m_vertexToHalfEdge[vEnd] = baseIdx + 1;
                }
                else
            ...
            for (uint i = 0; i < 3; ++i)
            {
                CORE_ASSERT(m_halfEdgeList[m_triangleToHalfEdge[i]].m_leftTriIdx == t, "Inconsistent triangle index");
