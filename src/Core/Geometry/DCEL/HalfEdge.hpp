...
class[[deprecated]] RA_CORE_API HalfEdge : public Container::IndexedObject {
  public:
    /// CONSTRUCTOR
    HalfEdge( const Container::Index& index =
                  Container::Index::Invalid() ); // Build a HalfEdge with the given index
    HalfEdge(
        const Vertex_ptr& v, const HalfEdge_ptr& next = nullptr, const HalfEdge_ptr& prev = nullptr,
        const HalfEdge_ptr& twin = nullptr, const Face_ptr& f = nullptr,
...
#include <Core/Geometry/DCEL/HalfEdge.inl>
