...
class[[deprecated]] RA_CORE_API Face : public Container::IndexedObject {
  public:
    /// CONSTRUCTOR
    Face( const Container::Index& index =
              Container::Index::Invalid() ); // Build a Face with the given index
    Face( const HalfEdge_ptr& he,
          const Container::Index& index =
              Container::Index::Invalid() ); // Build a Face with the given index and having
