...
class[[deprecated]] RA_CORE_API FullEdge : public Container::IndexedObject {
  public:
    /// CONSTRUCTOR
    FullEdge( const Container::Index& index =
                  Container::Index::Invalid() ); // Build an Edge with the given index
    FullEdge( const HalfEdge_ptr& he,
              const Container::Index& index =
                  Container::Index::Invalid() ); // Build an Edge with the given index, having he as
                                                 // its halfedge
...
#include <Core/Geometry/DCEL/FullEdge.inl>
