...
using LaplacianMatrix = Math::Sparse;
...
LaplacianMatrix RA_CORE_API cotangentWeightLaplacian(
    const Container::VectorArray<Math::Vector3>& p, const Container::VectorArray<Triangle>& T );
 ...
 */
/// WARNING: THE IMPLEMENTATION COULD BE WRONG
Math::Vector3 RA_CORE_API uniformLaplacian( const Math::Vector3& v,
...
cotangentWeightLaplacian( const Math::Vector3& v, const Container::VectorArray<Math::Vector3>& p );
