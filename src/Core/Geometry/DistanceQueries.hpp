...
inline RA_CORE_API Scalar projectOnSegment( const Math::Vector3& q, const Math::Vector3& a, const Math::Vector3& ab );

/// Return the squared distance from point Q to the segment AB defined by point A and
/// vector AB = (B - A).
inline RA_CORE_API Scalar pointToSegmentSq( const Math::Vector3& q, const Math::Vector3& a, const Math::Vector3& ab );
...
struct PointToTriangleOutput {
    ...
    Math::Vector3 meshPoint;      //! the point hit on the mesh
