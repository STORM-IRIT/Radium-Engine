...
AreaMatrix oneRingArea( const Container::VectorArray<Math::Vector3>& p,
...
void oneRingArea( const Container::VectorArray<Math::Vector3>& p,
...
AreaMatrix barycentricArea( const Container::VectorArray<Math::Vector3>& p,
                            const Container::VectorArray<Triangle>& T ) {
    return ( ( 1.0f / 3.0f ) * oneRingArea( p, T ) );
}

void barycentricArea( const Container::VectorArray<Math::Vector3>& p,
                      const Container::VectorArray<Triangle>& T, AreaMatrix& A ) {
    oneRingArea( p, T, A );
...
AreaMatrix voronoiArea( const Container::VectorArray<Math::Vector3>& p,
                        const Container::VectorArray<Triangle>& T ) {
    AreaMatrix A( p.size(), p.size() );
    A.reserve( p.size() );
    for ( const auto& t : T )
...
AreaMatrix mixedArea( const Container::VectorArray<Math::Vector3>& p,
                      const Container::VectorArray<Triangle>& T ) {
    AreaMatrix A( p.size(), p.size() );
    A.reserve( p.size() );
    for ( const auto& t : T )
