...
RA_CORE_API TriangleMesh makeXNormalQuad( const Math::Vector2& halfExts = Math::Vector2( 0.5,
                                                                                         0.5 ) );

/// Create a 2D quad mesh given half extents, centered on the origin with y axis as normal
RA_CORE_API TriangleMesh makeYNormalQuad( const Math::Vector2& halfExts = Math::Vector2( 0.5,
                                                                                         0.5 ) );

/// Create a 2D quad mesh given half extents, centered on the origin with z axis as normal
RA_CORE_API TriangleMesh makeZNormalQuad( const Math::Vector2& halfExts = Math::Vector2( 0.5,
                                                                                         0.5 ) );
...
/// Create an axis-aligned cubic mesh with the given half extents, centered on the origin.
RA_CORE_API TriangleMesh makeSharpBox( const Math::Vector3& halfExts = Math::Vector3( 0.5, 0.5,
                                                                                      0.5 ) );
...
/// with given radius.
RA_CORE_API TriangleMesh makeCylinder( const Math::Vector3& a, const Math::Vector3& b,
                                       Scalar radius, uint nFaces = 32 );
...
RA_CORE_API TriangleMesh makeTube( const Math::Vector3& a, const Math::Vector3& b,
                                   Scalar outerRadius, Scalar InnerRadius, uint nFaces = 32 );

/// Create a cone approximation (n-faced pyramid) with base face centered on base, pointing
/// towards tip with given base radius.
RA_CORE_API TriangleMesh makeCone( const Math::Vector3& base, const Math::Vector3& tip,
                                   Scalar radius, uint nFaces = 32 );
