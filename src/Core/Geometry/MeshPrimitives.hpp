...
/**
 * Create a parametric torus mesh. The minor radius is the radius of the inside
 * of the tube and the major radius is the radius of the whole torus.
 * The torus will be centered at the origin and have Z as rotation axis.
...
/**
 * Create a cylinder approximation (n-faced prism) with base faces centered on
 * \p a and \p b with the given radius.
 */
RA_CORE_API TriangleMesh makeCylinder( const Vector3& a, const Vector3& b, Scalar radius,
                                       uint nFaces = 32 );

/**
 * Create a capsule with given cylinder length and radius along the z axis.
 * \note Total length is length + 2 * radius.
 */
RA_CORE_API TriangleMesh makeCapsule( Scalar length, Scalar radius, uint nFaces = 32 );

/**
 * Create a tube (empty cylinder) delimited by two radii, with bases centered on
 * \p a and \p b.
 * \warning \p outerRadius must be larger than \p innerRadius.
 */
RA_CORE_API TriangleMesh makeTube( const Vector3& a, const Vector3& b, Scalar outerRadius,
                                   Scalar InnerRadius, uint nFaces = 32 );

/**
 * Create a cone approximation (n-faced pyramid) with base face centered on \p base,
 * pointing towards \p tip with the given base radius.
 */
