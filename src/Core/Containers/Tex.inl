...
namespace Ra
{
    namespace Core
    {
        ...
        namespace
        {
            ...
            {
                template <typename T>
                static T interpolate(const Grid <T, N>& grid,         // grid from which values are read
        ...
        Tex<T,N>::Tex( const IdxVector& resolution,
                       const Vector& start,
                       const Vector& end )
        ...
        inline T Tex<T,N>::fetch( const Vector& v ) const
        {
            ...
            IdxVector size = this->sizeVector() - IdxVector::Ones(); // TODO check this code on borders of the grid
            IdxVector clamped_nearest = Ra::Core::Vector::clamp<IdxVector>( nearest, IdxVector::Zero(), size);
