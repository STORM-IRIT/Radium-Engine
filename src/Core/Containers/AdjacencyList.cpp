...
void AdjacencyList::pruneLeaves( std::vector<uint>& pruned,
...
VectorArray<Eigen::Matrix<uint, 2, 1>>
AdjacencyList::extractEdgeList( bool include_leaf ) const
{
...
AdjacencyList::ConsistencyStatus AdjacencyList::computeConsistencyStatus() const 
{
    ...
    {
        if ( m_parent.at(node) >= int(node) )
        {
            return ConsistencyStatus::WrongParentOrdering;
        }
        if ( m_parent.at(node) < -1 )
        {
