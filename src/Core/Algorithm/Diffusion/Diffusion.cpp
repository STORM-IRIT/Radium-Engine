        ...
        namespace Algorithm {
            ...
            void diffuseDelta( const Geometry::AdjacencyMatrix& A, const Delta& delta, const Scalar lambda, const uint iteration, ScalarValue& value ) {
                ...
                for( uint n = 0; n < iter; ++n ) {
                    ...
                    //            oneRingMeanValue /= ( Scalar )nonZero; //oneRingMeanValue /= ( Scalar )A.col( j ).nonZeros();
                    //            tmp.coeffRef( j, 0 ) = ( lambda * oneRingMeanValue ) + ( uv * ( 1.0 - lambda ) );
                    //        }
                    //        u.swap( tmp );
...
#pragma omp parallel for
                    for( int k = 0; k < A.outerSize(); ++k ) {
                        ...
                        if( nonZero > 0 ) {
                            ...
                            Scalar s = ( lambda * oneRingMeanValue ) + ( delta.coeff( k, 0 ) * ( 1.0 - lambda ) );
#pragma omp critical
                            {
                                tmp.coeffRef( k, 0 ) = s;
                            }
