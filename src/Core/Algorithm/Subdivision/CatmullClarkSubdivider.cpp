...
bool CatmullClarkSubdivider::prepare( TopologicalMesh& mesh ) {
    ...
    createAllPropsOnFaces( mesh, m_normalPropF, m_floatPropsF, m_vec2PropsF, m_vec3PropsF,
                           m_vec4PropsF );
...
bool CatmullClarkSubdivider::cleanup( TopologicalMesh& mesh ) {
    ...
    clearAllProps( mesh, m_normalPropF, m_floatPropsF, m_vec2PropsF, m_vec3PropsF, m_vec4PropsF );
...
#pragma omp critical
            ...
            interpolateAllPropsOnFaces( mesh, fh, m_normalPropF, m_floatPropsF, m_vec2PropsF,
...
#pragma omp parallel for
    ...
    for ( auto f_itr = mesh.faces_begin(); f_itr != f_end; ++f_itr )
    {
        ...
        // deal with properties
        copyAllProps( mesh, heh3, heh5 );
        copyAllProps( mesh, heh1, heh6 );
...
void CatmullClarkSubdivider::split_face( TopologicalMesh& mesh,
                                         const TopologicalMesh::FaceHandle& fh, int iter ) {
    /*
    ...
    */
    ...
    copyAllPropsFromFace( mesh, fh, hold, m_normalPropF, m_floatPropsF, m_vec2PropsF, m_vec3PropsF,
                          m_vec4PropsF );
    ...
    for ( size_t i = 1; i < valence; i++ )
    {
        ...
        copyAllPropsFromFace( mesh, fh, hnew, m_normalPropF, m_floatPropsF, m_vec2PropsF,
                              m_vec3PropsF, m_vec4PropsF );
...
void CatmullClarkSubdivider::split_edge( TopologicalMesh& mesh,
                                         const TopologicalMesh::EdgeHandle& eh, int iter ) {
...
#pragma omp parallel for
        for ( int j = 0; j < m_newFaceVertexOps[i].size(); ++j )
        {
...
#pragma omp parallel for
        for ( int j = 0; j < m_newEdgeVertexOps[i].size(); ++j )
        {
            Ra::Core::Vector3 pos( 0, 0, 0 );
...
#pragma omp parallel for
    ...
    // way)
#pragma omp parallel for
    for ( int j = 0; j < m_triangulationPropOps.size(); ++j )
    {
        Ra::Core::Vector3 nor( 0, 0, 0 );
