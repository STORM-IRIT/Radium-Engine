...
// FIXME(Charly): Change el to Ra::Core
...
private:
    ...
    inline Conf_T unsafeGetConfigByVal(Level level, const std::map<Level, Conf_T>* confMap, const char* confName) {
        ...
        typename std::map<Level, Conf_T>::const_iterator it2= confMap->find(Level::Global);
        if (it == confMap->end()) {
            if (it2 == confMap->end()) {
    ...
    inline Conf_T& unsafeGetConfigByRef(Level level, std::map<Level, Conf_T>* confMap, const char* confName) {
        ...
        typename std::map<Level, Conf_T>::iterator it2= confMap->find(Level::Global);
        if (it == confMap->end()) {
            if (it2 == confMap->end()) {
                ELPP_INTERNAL_ERROR("Unable to get configuration [" << confName << "] for level ["
                    << LevelHelper::convertToString(level) << "]"
