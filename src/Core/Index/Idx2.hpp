...
namespace Ra
{
    namespace Core
    {
        ...
        struct Idx2
        {
            ...
            inline
            void set_2d(const Vector2i& p){ set_2d(p(0), p(1)); }
            ...
            inline
            Vector2i to_2d() const { Vector2i r; to_2d(r(0), r(1)); return r; }

            inline
            Vector2i to_Vector2i() const { return to_2d(); }
        ...
        #ifdef __CUDACC__
            int3 to_int3() const { return make_int3(_size(0), _size(0), _id); }
        #endif
            ...
            inline
            Idx2 operator++(   ) { return Idx2(_size, ++_id); }
            ...
            inline
            Idx2 operator--(int) { return Idx2(_size, _id--); }
        ...
        private:
        ...
        #if 0
            bool operator<=(const Idx2& ) const { return false; }
            bool operator>=(const Idx2& ) const { return false; }
            bool operator< (const Idx2& ) const { return false; }
            bool operator> (const Idx2& ) const { return false; }

            Idx2 operator- (const Idx2& ) const { return Idx2(); }
            Idx2 operator+ (const Idx2& ) const { return Idx2(); }
            Idx2 operator+=(const Idx2& )       { return Idx2(); }
            Idx2 operator-=(const Idx2& )       { return Idx2(); }
            ...
            Idx2 operator+ (int )  const { return Idx2(); }
            Idx2 operator- (int )  const { return Idx2(); }
            Idx2 operator+=(int )        { return Idx2(); }
            Idx2 operator-=(int )        { return Idx2(); }
        #endif
        };
    }
