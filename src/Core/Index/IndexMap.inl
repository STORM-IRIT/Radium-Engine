...
/// CONSTRUCTOR
template <typename T>
IndexMap<T>::IndexMap() { m_data.clear(); m_free.push_back( Index( 0 ) ); }

template <typename T>
IndexMap<T>::IndexMap( const IndexMap& id_map ) { m_data = id_map.m_data; m_free = id_map.m_free; }
...
inline bool IndexMap<T>::insert( const T& obj, Index& idx ) {
    if( !remove_free_index( idx ) ) return false;
    IndexMapEntry imp( idx, obj );
    typename std::deque< IndexMapEntry >::iterator it = std::lower_bound( m_data.begin(), m_data.end(), imp );
    m_data.insert( it, imp );
    return true;
...
inline bool IndexMap<T>::remove( const Index& idx ) {
    ...
    if ( data_it == m_data.end() ) return false;
...
/// QUERY
template <typename T>
inline bool IndexMap<T>::empty() const { return  m_data.empty(); }

template <typename T>
inline bool IndexMap<T>::full()  const { return m_free.empty(); }
...
/// OPERATOR
template <typename T>
inline T& IndexMap<T>::operator[]( const Index& idx ) { return access( idx ); }

template <typename T>
inline T& IndexMap<T>::operator[]( const int i ) { return access( i );   }

template <typename T>
inline Index& IndexMap<T>::operator<<( const T& obj ) { return insert( obj ); }

template <typename T>
inline IndexMap<T>& IndexMap<T>::operator>>( const Index& idx ) { remove( idx ); return *this; }
