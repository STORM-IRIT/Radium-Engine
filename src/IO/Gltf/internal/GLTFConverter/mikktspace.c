...
tbool genTangSpace( const SMikkTSpaceContext* pContext, const float fAngularThreshold ) {
    ...
    iNrTSPaces =
        GenerateInitialVerticesIndexList( pTriInfos, piTriListIn, pContext, iNrTrianglesIn );
    ...
    for ( f = 0; f < iNrFaces; f++ ) {
        ...
        for ( i = 0; i < verts; i++ ) {
            ...
            if ( pContext->m_pInterface->m_setTSpace != NULL ) {
                ...
                pContext->m_pInterface->m_setTSpace( pContext,
                                                     tang,
                                                     bitang,
                                                     pTSpace->fMagS,
                                                     pTSpace->fMagT,
...
static void BuildNeighborsFast( STriInfo pTriInfos[],
                                ...
                                const int iNrTrianglesIn );
static void
BuildNeighborsSlow( STriInfo pTriInfos[], const int piTriListIn[], const int iNrTrianglesIn );
...
static tbool GenerateTSpaces( STSpace psTspace[],
    ...
    for ( int g = 0; g < iNrActiveGroups; g++ )
    ...
    for ( int g = 0; g < iNrActiveGroups; g++ ) {
        ...
        for ( int i = 0; i < pGroup->iNrFaces; i++ ) // triangles
        {
...
static void
QuickSortEdges( SEdge* pSortBuffer, int iLeft, int iRight, const int channel, unsigned int uSeed );
...
static void BuildNeighborsFast( STriInfo pTriInfos[],
    ...
    unsigned int uSeed = INTERNAL_RND_SORT_SEED; // could replace with a random seed?
    int iEntries = 0, iCurStartIndex = -1;
    for ( int f = 0; f < iNrTrianglesIn; f++ )
        for ( int i = 0; i < 3; i++ ) {
