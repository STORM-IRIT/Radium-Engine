...
void AssimpLightDataLoader::loadLightData( const aiScene* scene, const aiLight& light,
    ...
    setFrame( frame );
    Core::Math::Color color( light.mColorDiffuse.r, light.mColorDiffuse.g, light.mColorDiffuse.b,
                             1.0 );
    ...
    {
        Core::Math::Vector4 dir( light.mDirection[0], light.mDirection[1], light.mDirection[2],
                                 0.0 );
    ...
    case Core::Asset::LightData::AREA_LIGHT:
    {
        LOG( Core::Utils::logWARNING )
            << "Light " << data.getName() << " : AREA light are not yet supported.";
    }
    break;
    default:
