...
Ra::Core::Asset::FileData* AsciiPointCloudLoader::loadFile( const string& filename ) {
    using Ra::Core::Asset::GeometryData;
		...
		static int id = 0;
		auto geometry = make_unique<GeometryData>("TRAJ_PC_" + to_string(++id), GeometryData::POINT_CLOUD);
        if (geometry == nullptr) {
        ...
        string temp;
        while (words >> temp) { tokens.emplace_back(temp); };
        ...
        for (size_t i = 0;i < tokens.size();++i) {
            ...
            attribs.emplace_back(geometry->getGeometry().vertexAttribs().addAttrib<Scalar>( tokens[i] ));
        }
        ...
        while (stream >> num) {
            ...
            if (i == (int)(tokens.size() - 1)) {
                vertices.emplace_back(Scalar(pointRecord[xpos]), Scalar(pointRecord[ypos]), Scalar(pointRecord[zpos]));
                size_t k = 0;
                for (size_t j = 0;j < tokens.size();++j) {
                    if (((int)j == xpos) || ((int)j == ypos) || ((int)j == zpos)) { continue; }
                    geometry->getGeometry().vertexAttribs().getDataWithLock(attribs[k]).emplace_back(Scalar(pointRecord[j]));
