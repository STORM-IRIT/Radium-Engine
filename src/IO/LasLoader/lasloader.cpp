...
Ra::Core::Asset::FileData* LasLoader::loadFile( const string& filename ) {
    ...
    for (unsigned int i = 0;i < nb_data;++i) {
        ...
        if (colors) {
            ...
            auto handle_colors = geometry->getGeometry().vertexAttribs().findAttrib<Ra::Core::Vector4>(
                        Ra::Core::Geometry::getAttribName(Ra::Core::Geometry::MeshAttrib::VERTEX_COLOR));
            geometry->getGeometry().vertexAttribs().getDataWithLock(handle_colors).emplace_back(Scalar(red), Scalar(green), Scalar(blue), 1_ra);
            geometry->getGeometry().vertexAttribs().unlock(handle_colors);
        }

        if (gps_time) {
            //computing GPS time if found
            auto handle_time = geometry->getGeometry().vertexAttribs().findAttrib<Scalar>("gps_time");
            auto& time = geometry->getGeometry().vertexAttribs().getDataWithLock(handle_time);
            if (data_format == 6) {
