...
Ra::Core::Asset::FileData* LasLoader::loadFile( const string& filename ) {
    ...
    VectorArray<Ra::Core::Vector4>& color =
        geometry->getGeometry().vertexAttribs().getDataWithLock( handle_color );
    ...
    VectorArray<Scalar>& time =
        geometry->getGeometry().vertexAttribs().getDataWithLock( handle_time );
    ...
    if ( !colors ) { geometry->getGeometry().vertexAttribs().removeAttrib( handle_color ); }
