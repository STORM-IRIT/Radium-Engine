...
void TimelineFrameSelector::onAddingKeyFrame( Scalar time, bool internal ) {
    ...
    int nbKeyFrames = static_cast<int>( m_keyFrames.size() );
    m_keyFrames.insert( time );

    // if keyFrame not already here
    if ( static_cast<int>( m_keyFrames.size() ) != nbKeyFrames )
    {
        updateCursorSpin();
    ...
    {
        if ( internal ) { emit keyFrameChanged( time ); }
...
void TimelineFrameSelector::onDeletingKeyFrame( bool internal ) {
    auto it = m_keyFrames.find( m_cursor );
    ...
    updateNbKeyFrameSpin();
    if ( internal ) { emit keyFrameDeleted( m_cursor ); }
...
void TimelineFrameSelector::onMoveKeyFrame( Scalar time0, Scalar time1, bool internal ) {
    ...
    m_keyFrames.erase( time0 );
    m_keyFrames.insert( m_cursor );

    updateCursorSpin();

    if ( internal ) { emit keyFrameMoved( time0, time1 ); }
...
void TimelineFrameSelector::onMoveKeyFrames( Scalar time, Scalar offset, bool internal ) {
    ...
    auto it = m_keyFrames.find( time );
    if ( it == m_keyFrames.end() ) { return; }

    std::set<Scalar> clone;
    std::transform( m_keyFrames.begin(),
                    m_keyFrames.end(),
                    std::inserter( clone, clone.end() ),
    ...
    if ( internal ) { emit keyFramesMoved( time, offset ); }
    ...
    else if ( event->button() == Qt::RightButton )
    {
        ...
        if ( it != m_keyFrames.end() )
        {
            ...
            if ( shiftDown )
            {
                ...
                if ( m_keyFrames.find( nearest ) == m_keyFrames.end() &&
        ...
        else
        {
            ...
            if ( shiftDown )
            {
                auto itRight = m_keyFrames.lower_bound( newFrame );
            ...
            {
                auto itLeft = --m_keyFrames.lower_bound( newFrame );
...
void TimelineFrameSelector::updateCursorSpin() {
    if ( m_keyFrames.find( m_cursor ) != m_keyFrames.end() )
    {
    ...
    while ( it != m_keyFrames.end() )
    {
        ...
        if ( keyFrame >= left )
        {
            ...
            if ( keyFrame > right )
            {
                ...
                m_keyFrames.insert( keyFrame - dist );
            }
            else
