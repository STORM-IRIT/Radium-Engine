#ifndef RADIUMENGINE_TIMELINE_SCROLLAREA_H
#define RADIUMENGINE_TIMELINE_SCROLLAREA_H
...
class TimelineScrollArea : public QScrollArea
{
  ...
  private:
    ...
    int m_sliderPos; ///< x coordinate of the slider on mouse middle click.

    Scalar m_maxDuration{20_ra}; ///< <b>end<\b> of the Timeline's playzone.
    Scalar m_step;        ///< Step between two scale graduations.
    int m_nbInterval{0};  ///< Number of scale graduations.
    Scalar m_pixPerSec;   ///< Number of pixels used to display 1 second on the scale.
    int m_zero;           ///< Pixel corresponding to time 0.
...
#endif // RADIUMENGINE_TIMELINE_SCROLLAREA_H
