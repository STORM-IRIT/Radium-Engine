...
void BaseApplication::setupScene() {
    ...
    auto frame = Primitive( Engine::SystemEntity::uiCmp(),
                            Frame( Ra::Core::Math::Transform::Identity(), 0.05f ) );
...
void BaseApplication::recordFrame() {
    std::string filename;
    Ra::Core::Utils::stringPrintf( filename, "%s/radiumframe_%06u.png", m_exportFoldername.c_str(),
                                   m_frameCounter );
        ...
        if ( ext == sysDllExt )
        {
                 ...
                 std::find_if( loadList.begin(), loadList.end(), stringCmp ) == loadList.end() )
            {
                LOG( Core::Utils::logDEBUG )
                    << "Ignoring " << filename.toStdString() << " (not on load list)";
                continue;
            }
            ...
            {
                LOG( Core::Utils::logDEBUG )
                    << "Ignoring " << filename.toStdString() << " (on ignore list)";
                continue;
            }
            ...
            {
                LOG( Core::Utils::logERROR )
                    << "Something went wrong while trying to load plugin " << filename.toStdString()
