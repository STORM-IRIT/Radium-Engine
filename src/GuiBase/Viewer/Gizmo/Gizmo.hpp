...
class Gizmo {
  ...
  public:
    RA_CORE_ALIGNED_NEW
    Gizmo( Engine::Component* c, const Core::Math::Transform& worldTo,
    ...
    virtual void setInitialState( const Engine::Camera& cam,
                                  const Core::Math::Vector2& initialXY ) = 0;

    /// Called when the mose movement is recorder with the camera parameters and the current pixel
    /// coordinates.
    virtual Core::Math::Transform mouseMove( const Engine::Camera& cam,
                                             const Core::Math::Vector2& nextXY,
