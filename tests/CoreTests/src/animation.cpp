...
void run() {
    ...
    RA_VERIFY( !Ra::Core::Animation::normalizeWeights( matrix1 ), "Matrix1 is already normalized" );

    matrix1.coeffRef( w / 3, h / 2 ) = 0.8;

    RA_VERIFY( Ra::Core::Animation::normalizeWeights( matrix1 ), "Matrix1 normalization ok" );
    ...
    RA_VERIFY( Ra::Core::Animation::normalizeWeights( matrix2 ), "Matrix2 needs to be normalized" );

    RA_VERIFY( !Ra::Core::Animation::normalizeWeights( matrix3 ), "Matrix3 is already normalized" );

    RA_VERIFY( matrix1.isApprox( matrix2 ), "Two matrices are equivalent after normalization" );
