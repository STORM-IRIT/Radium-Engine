...
#define RA_VERIFY(a, DESC) Ra::Testing::verify_impl(a, \
                  Ra::Testing::g_test_stack.back().c_str(), __FILE__, \
                  __LINE__, DESC)//RADIUM_PP_MAKE_STRING(a))
...
#define CALL_SUBTEST(FUNC) do { \
    ...
    FUNC; \
    MYPRAGMA("omp critical") \
    { Ra::Testing::g_test_stack.pop_back(); } \
  } while (0)
...
static bool init_testing(int argc, const char *argv[])
{
  ...
  if(need_help)
  {
    ...
    std::cout << std::endl;
    std::cout << "If defined, the environment variables RADIUM_REPEAT and RADIUM_SEED" << std::endl;
