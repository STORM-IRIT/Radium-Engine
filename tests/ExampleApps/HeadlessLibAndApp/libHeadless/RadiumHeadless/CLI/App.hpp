...
class App
{
  ...
  public:
    ...
    App* disabled_by_default( bool disable = true ) {
        if ( disable ) { default_startup = startup_mode::disabled; }
        else
        {
    ...
    App* enabled_by_default( bool enable = true ) {
        if ( enable ) { default_startup = startup_mode::enabled; }
        else
        {
    ...
    Option* add_option( std::string option_name,
        ...
        throw(
            OptionAlreadyAdded( "added option matched existing option name" ) ); // LCOV_EXCL_LINE
    }
  ...
  private:
    ...
    Option*
    _add_flag_internal( std::string flag_name, CLI::callback_t fun, std::string flag_description ) {
        Option* opt;
  ...
  public:
    ...
    std::size_t count_all() const {
        ...
        { cnt += sub->count_all(); }
        if ( !get_name().empty() )
        { // for named subcommands add the number of times the subcommand
          // was called
            cnt += parsed_;
        }
        return cnt;
    ...
    std::string get_display_name( bool with_aliases = false ) const {
        ...
        if ( aliases_.empty() || !with_aliases || aliases_.empty() ) { return name_; }
  ...
  protected:
    ...
    void run_callback( bool final_mode = false ) {
        ...
        for ( auto& subc : subcommands_ )
        {
            if ( subc->name_.empty() && subc->count_all() > 0 ) { subc->run_callback( true ); }
        }

        // finally run the main callback
        if ( final_callback_ && ( parsed_ > 0 ) )
        {
            if ( !name_.empty() || count_all() > 0 ) { final_callback_(); }
        }
    }
    ...
    void _process_config_file() {
        if ( config_ptr_ != nullptr )
        {
            ...
            {
                if ( config_required ) { throw FileError::Missing( "no specified config file" ); }
                return;
            }
            for ( auto rit = config_files.rbegin(); rit != config_files.rend(); ++rit )
            {
                ...
                if ( path_result == detail::path_type::file )
                {
                    try
                    {
                        ...
                        if ( !file_given ) { config_ptr_->add_result( config_file ); }
                    }
    ...
    bool _parse_single_config( const ConfigItem& item, std::size_t level = 0 ) {
        ...
        if ( op == nullptr ) { op = get_option_no_throw( item.name ); }
        if ( op == nullptr )
        {
...
inline void retire_option( App* app, Option* opt ) {
    ...
    auto opt2 =
        app->add_option( option_copy->get_name( false, true ),
