...
macro(add_external_folder NAME FOLDER)
    ...
    execute_process(COMMAND ${CMAKE_COMMAND} -E make_directory ${EXT_WORKING_DIR}/cmake)
    ...
    if(DEFINED CACHED_INSTALL_PREFIX)
        if(NOT "${CACHED_INSTALL_PREFIX}" STREQUAL "${CMAKE_INSTALL_PREFIX}")
            message(
                STATUS
                    "CMAKE_INSTALL_PREFIX has changed (from ${CACHED_INSTALL_PREFIX} to ${CMAKE_INSTALL_PREFIX}), reinstalling dependencies"
            )
            set(UPDATE_EXTERNAL ON)
        endif()
    endif()
    ...
    if(NOT EXISTS "${CMAKE_INSTALL_PREFIX}/share/Radium-${NAME}.touch")
        message(
            STATUS
                "CMAKE_INSTALL_PREFIX is empty (${CMAKE_INSTALL_PREFIX}), reinstalling dependencies"
        )
        set(UPDATE_EXTERNAL ON)
    endif()
    ...
    if(UPDATE_EXTERNAL)
        ...
        execute_process(
            COMMAND ${CMAKE_COMMAND} ${FOLDER} ${RADIUM_EXTERNAL_CMAKE_OPTIONS}
                    -DCMAKE_MESSAGE_INDENT=${indent_string}\; -Wno-dev ${ARGN}
            WORKING_DIRECTORY ${EXT_WORKING_DIR} RESULT_VARIABLE ret
        )
        if(NOT ret EQUAL "0")
        ...
        message(STATUS "EXEC")
        execute_process(
            COMMAND ${CMAKE_COMMAND} --build . --config ${CMAKE_BUILD_TYPE} --target
                    ${RadiumExternalMakeTarget} WORKING_DIRECTORY ${EXT_WORKING_DIR}
            RESULT_VARIABLE ret
        )
        ...
        option(RADIUM_SKIP_${NAME_UPPER}_EXTERNAL
               "Skip updating ${NAME}::external (disable for rebuild)" ON
        )

    else()
        message(
            STATUS
                "Skip ${NAME}::external update (use RADIUM_SKIP_${NAME_UPPER}_EXTERNAL to change behavior)"
        )
    endif()
    ...
    install(FILES "${CMAKE_CURRENT_BINARY_DIR}/Radium-${NAME}.touch" DESTINATION share/)
