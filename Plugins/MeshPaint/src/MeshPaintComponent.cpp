...
void MeshPaintComponent::initialize() {
    ...
    if ( !geometryData )
    {
        LOG( Ra::Core::Utils::logWARNING )
            << "Cannot initialize MeshPaint Component: no geometry data." << std::endl;
        return;
    }

    m_renderObjectReader =
        compMess->getterCallback<Ra::Core::Container::Index>( getEntity(), m_dataId );
    ...
    m_paintColors.resize( ro->getMesh()->getGeometry().m_vertices.size(), Ra::Core::Math::Skin() );
