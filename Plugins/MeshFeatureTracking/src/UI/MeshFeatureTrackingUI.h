...
struct FeatureData {
    Ra::Engine::Renderer::PickingMode
        m_mode; // feature type: only vertex, edge, triangles are supported.
    std::array<int, 4>
        m_data; // vertices indices in 0-2 according to m_mode, plus triangle idx in 3 for triangles
};
...
class MeshFeatureTrackingUI : public QFrame {
  ...
  public:
    ...
    void updateTracking( const FeatureData& data, const Ra::Core::Math::Vector3& pos,
