...
class FM_PLUGIN_API FancyMeshComponent : public Ra::Engine::Component {
  ...
  private:
    // The duplicate table for vertices, according to vertices position and
    // normals in the mesh data. Let M be a mesh with a total of n vertices, of
    // which m are duplicates of others (same position but different attributes).
    // Then, the duplicate table for M has n entries such that m_duplicateTable[i]
    // is the index for the duplicated attributes of vertice i in M data
    // (position, one-ring normal, ...) while i is the index for its non duplicated
    // attributes (texture coordinates, face normal, tangent vector, ...).
