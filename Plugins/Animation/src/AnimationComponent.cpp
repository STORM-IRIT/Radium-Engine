    ...
    for ( uint i = 0; i < m_skel.size(); ++i )
    {
        ...
        } else
        {
            LOG( Ra::Core::Utils::logDEBUG )
                << "Bone " << m_skel.getLabel( i ) << " not displayed.";
        }
    }
}
...
void AnimationComponent::handleSkeletonLoading(
    const Ra::Core::Asset::HandleData* data,
    const std::vector<Ra::Core::Container::Index>& duplicateTable, uint nbMeshVertices ) {
...
void AnimationComponent::createWeightMatrix(
    const Ra::Core::Asset::HandleData* data, const std::map<uint, uint>& indexTable,
    const std::vector<Ra::Core::Container::Index>& duplicateTable, uint nbMeshVertices ) {
...
Ra::Core::Math::Transform
AnimationComponent::getTransform( Ra::Core::Container::Index roIdx ) const {
