...
//------------------------------------------------------------------
PBS::PBS() : m_volumetricMeshValid{false}, m_gravity{0.0, -9.81, 0.0}, m_maxIter{12} {}
...
/* -----------------------------------
 ...
 ------------------------------------- */
Scalar PBS::distancePointSegment( const Vector3& point, const Vector3& startSegment,
                                  const Vector3& endSegment ) {
    Scalar distance;
...
void PBS::smoothWeights( const Graph::AdjacencyList& skeletonGraph,
                         const unsigned int indexParticle, const Vector3& particle,
                         const unsigned int indexNearestBone, const Vector3& startBone,
                         const Vector3& endBone ) {
    ...
    } else
    {
        ...
        for ( auto it = indicesChildrenNearestBone.cbegin();
              it != indicesChildrenNearestBone.cend(); ++it )
        {
            m_weightParticles.coeffRef( indexParticle, *it ) = weight;
        }
    }
}
void PBS::weightParticle( const Graph::AdjacencyList& skeletonGraph,
                          const unsigned int indexParticle, const Vector3& particle,
                          const unsigned int indexNearestBone, const Vector3& startBone,
                          const Vector3& endBone ) {
    ...
    {
        smoothWeights( skeletonGraph, indexParticle, particle, indexNearestBone, startBone,
                       endBone );
    }
}
...
void PBS::weightParticlesBindConstraint() {
    ...
    for ( auto i = 0; i < num_particles; ++i )
    {
        ...
        searchNearestBone( particle_position, startBone, endBone, indexNearestBone,
                           distanceNearestBone );
        // Attribute Weight
...
Scalar PBS::computeDeterminant( const Vector3& vertex0, const Vector3& vertex1,
    ...
    return ( 1.0f / 6.0f ) *
           ( vertex3 - vertex0 ).dot( ( vertex2 - vertex0 ).cross( vertex1 - vertex0 ) );
}
void PBS::getDeterminants( const Vector3& tetVertex0, const Vector3& tetVertex1,
                           const Vector3& tetVertex2, const Vector3& tetVertex3,
                           const Vector3& surfaceVertex, Scalar& determinant0, Scalar& determinant1,
...
const Vector4 PBS::getBarycentricCoordinate( const Scalar determinant0, const Scalar determinant1,
                                             const Scalar determinant2, const Scalar determinant3,
...
If by chance the Determinant det0 is 0, then your tetrahedron is degenerate (the points are
coplanar). If any other Di=0, then P lies on boundary i (boundary i being that boundary formed by
the three points other than Vi). If the sign of any Di differs from that of D0 then P is outside
boundary i. If the sign of any Di equals that of D0 then P is inside boundary i. If P is inside all
4 boundaries, then it is inside the tetrahedron. As a check, it must be that D0 = D1+D2+D3+D4. The
pattern here should be clear; the computations can be extended to simplicies of any dimension. (The
2D and 3D case are the triangle and the tetrahedron). If it is meaningful to you, the quantities bi
= Di/D0 are the usual barycentric coordinates. Comparing signs of Di and D0 is only a check that P
and Vi are on the same side of boundary i.
*/
bool PBS::checkPointInTetra( Scalar determinant0, Scalar determinant1, Scalar determinant2,
                             Scalar determinant3, Scalar determinant4 ) {
    bool insideTetra = ( determinant0 != 0 );
    insideTetra &=
        ( ( determinant0 < 0 ) && ( ( ( determinant1 < 0 ) && ( determinant2 < 0 ) &&
                                      ( determinant3 < 0 ) && ( determinant4 < 0 ) ) ) ) ||
        ( ( determinant0 > 0 ) && ( ( determinant1 > 0 ) && ( determinant2 > 0 ) &&
                                    ( determinant3 > 0 ) && ( determinant4 > 0 ) ) );
    return insideTetra;
}
Scalar PBS::getSquaredDistancePointToTetrahedron( const Vector3& tetVertex0,
                                                  const Vector3& tetVertex1,
                                                  const Vector3& tetVertex2,
...
// Update Surface Mesh
//---------------------------------
Vector3 PBS::getPositionByBarycentricCoord( const TetModel::Tets& tetras, const ParticleData& pd,
