...
void MathFunctions::jacobiRotate( Matrix3r& A, Matrix3r& R, int p, int q ) {
    ...
    Real t = 1.0f / ( std::abs( d ) + sqrt( d * d + 1.0f ) );
    if ( d < 0.0f )
        t = -t;
...
// ----------------------------------------------------------------------------------------------
void MathFunctions::eigenDecomposition( const Matrix3r& A, Matrix3r& eigenVecs,
                                        Vector3r& eigenVals ) {
...
void MathFunctions::polarDecomposition( const Matrix3r& A, Matrix3r& R, Matrix3r& U, Matrix3r& D ) {
    ...
    Matrix3r S1;
    S1( 0, 0 ) =
        l0 * U( 0, 0 ) * U( 0, 0 ) + l1 * U( 0, 1 ) * U( 0, 1 ) + l2 * U( 0, 2 ) * U( 0, 2 );
    S1( 1, 1 ) =
        l0 * U( 1, 0 ) * U( 1, 0 ) + l1 * U( 1, 1 ) * U( 1, 1 ) + l2 * U( 1, 2 ) * U( 1, 2 );
    S1( 2, 2 ) =
        l0 * U( 2, 0 ) * U( 2, 0 ) + l1 * U( 2, 1 ) * U( 2, 1 ) + l2 * U( 2, 2 ) * U( 2, 2 );

    S1( 0, 1 ) =
        l0 * U( 0, 0 ) * U( 1, 0 ) + l1 * U( 0, 1 ) * U( 1, 1 ) + l2 * U( 0, 2 ) * U( 1, 2 );
    S1( 0, 2 ) =
        l0 * U( 0, 0 ) * U( 2, 0 ) + l1 * U( 0, 1 ) * U( 2, 1 ) + l2 * U( 0, 2 ) * U( 2, 2 );
    S1( 1, 2 ) =
        l0 * U( 1, 0 ) * U( 2, 0 ) + l1 * U( 1, 1 ) * U( 2, 1 ) + l2 * U( 1, 2 ) * U( 2, 2 );
...
/** Perform a polar decomposition of matrix M and return the rotation matrix R. This method handles
 * the degenerated cases.
 */
void MathFunctions::polarDecompositionStable( const Matrix3r& M, const Real tolerance,
                                              Matrix3r& R ) {
    ...
    do
    {
        ...
        Real det =
            Mt( 0, 0 ) * MadjTt( 0, 0 ) + Mt( 0, 1 ) * MadjTt( 0, 1 ) + Mt( 0, 2 ) * MadjTt( 0, 2 );
        ...
        if ( std::abs( det ) < 1.0e-12 )
        {
            ...
            } else
            {
                ...
                MadjTt.row( ( index + 1 ) % 3 ) =
                    Mt.row( ( index + 2 ) % 3 ).cross( Mt.row( index ) );
                MadjTt.row( ( index + 2 ) % 3 ) =
                    Mt.row( index ).cross( Mt.row( ( index + 1 ) % 3 ) );
                Matrix3r M2 = Mt.transpose();
                Mone = oneNorm( M2 );
                Minf = infNorm( M2 );
                det = Mt( 0, 0 ) * MadjTt( 0, 0 ) + Mt( 0, 1 ) * MadjTt( 0, 1 ) +
                      Mt( 0, 2 ) * MadjTt( 0, 2 );
            }
        }
        ...
        const Real gamma =
            sqrt( sqrt( ( MadjTone * MadjTinf ) / ( Mone * Minf ) ) / std::abs( det ) );
 ...
 */
void MathFunctions::svdWithInversionHandling( const Matrix3r& A, Vector3r& sigma, Matrix3r& U,
                                              Matrix3r& VT ) {
    ...
    {
        Vector3r sigmaInv( 1.0f / sigma[0], 1.0f / sigma[1], 1.0f / sigma[2] );
        U = A * V;
        for ( unsigned char l = 0; l < 3; l++ )
        {
            for ( unsigned char m = 0; m < 3; m++ )
