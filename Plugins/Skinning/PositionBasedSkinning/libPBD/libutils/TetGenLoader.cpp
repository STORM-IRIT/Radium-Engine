...
void TetGenLoader::loadTetFile( const std::string& filename, std::vector<Vector3r>& vertices,
...
void TetGenLoader::loadTetgenModel( const std::string& nodeFilename, const std::string& eleFilename,
    ...
    for ( i = 0; i < num_tetras; ++i )
    {
        ...
        getline( finEle, eleLine );
        sStream << eleLine;
        sStream >> eleInd >> tets[4 * i + 0] >> tets[4 * i + 1] >> tets[4 * i + 2] >>
            tets[4 * i + 3];
...
void TetGenLoader::loadMSHModel( const std::string& mshFilename, std::vector<Vector3r>& vertices,
                                 std::vector<unsigned int>& tets ) {
