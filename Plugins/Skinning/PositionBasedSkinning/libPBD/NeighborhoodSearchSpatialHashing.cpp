...
NeighborhoodSearchSpatialHashing::NeighborhoodSearchSpatialHashing(
    const unsigned int numParticles, const Real radius, const unsigned int maxNeighbors,
...
void NeighborhoodSearchSpatialHashing::cleanup() {
    ...
    {
        Hashmap<NeighborhoodSearchCellPos*,
                NeighborhoodSearchSpatialHashing::HashEntry*>::KeyValueMap* kvMap =
            m_gridMap.getKeyValueMap( i );
        if ( kvMap )
        {
            for ( Hashmap<NeighborhoodSearchCellPos*,
                          NeighborhoodSearchSpatialHashing::HashEntry*>::KeyValueMap::iterator
                      iter = kvMap->begin();
                  iter != kvMap->end(); iter++ )
            {
...
void NeighborhoodSearchSpatialHashing::neighborhoodSearch( Vector3r* x ) {
    const Real factor = 1.0f / m_cellGridSize;
        ...
        for ( int i = 0; i < (int)m_numParticles; i++ )
        {
            ...
            for ( unsigned char j = 0; j < 3; j++ )
            {
                for ( unsigned char k = 0; k < 3; k++ )
                {
                    ...
                    {
                        NeighborhoodSearchCellPos cellPos( cellPos1 + j, cellPos2 + k,
                                                           cellPos3 + l );
...
void NeighborhoodSearchSpatialHashing::neighborhoodSearch( Vector3r* x,
    ...
    {
        const int cellPos1 =
            NeighborhoodSearchSpatialHashing::floor( boundaryX[i][0] * factor ) + 1;
        const int cellPos2 =
            NeighborhoodSearchSpatialHashing::floor( boundaryX[i][1] * factor ) + 1;
        const int cellPos3 =
            NeighborhoodSearchSpatialHashing::floor( boundaryX[i][2] * factor ) + 1;
        NeighborhoodSearchCellPos cellPos( cellPos1, cellPos2, cellPos3 );
        HashEntry*& entry = m_gridMap[&cellPos];
                            ...
                            for ( unsigned int m = 0; m < ( *entry )->particleIndices.size(); m++ )
                            {
                                ...
                                if ( pi != i )
                                {
                                    ...
                                    if ( pi < m_numParticles )
                                        ...
                                        dist2 =
                                            ( x[i] - boundaryX[pi - m_numParticles] ).squaredNorm();
