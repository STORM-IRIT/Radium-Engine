...
class PositionBasedRigidBodyDynamics {
    ...
    // -----------------------------------------------------
  private:
    static void computeMatrixK( const Vector3r& connector, const Real invMass, const Vector3r& x,
                                const Matrix3r& inertiaInverseW, Matrix3r& K );

    static void computeMatrixK( const Vector3r& connector0, const Vector3r& connector1,
                                const Real invMass, const Vector3r& x,
                                const Matrix3r& inertiaInverseW, Matrix3r& K );
  ...
  public:
    ...
    static bool solve_BallJoint(
        ...
        Vector3r& corr_x0, Quaternionr& corr_q0, Vector3r& corr_x1, Quaternionr& corr_q1 );
     ...
     * A hinge joint removes three translational and two rotational  degrees of freedom between the
     * bodies. The hinge joint info must be generated in the initialization process of the model by
     * calling the function init_HingeJoint() and updated each time the bodies change their state by
     * update_HingeJoint().\n\n More information can be found in: \cite Deul2014
     *
     ...
     * A universal joint removes three translational and one rotational degree of freedom between
     * the bodies. The universal joint info must be generated in the initialization process of the
     * model by calling the function init_UniversalJoint() and updated each time the bodies change
     * their state by update_UniversalJoint().\n\n More information can be found in: \cite Deul2014
     *
     ...
     * A slider joint removes two translational and three rotational degrees of freedom between the
     * bodies. The slider joint info must be generated in the initialization process of the model by
     * calling the function init_SliderJoint() and updated each time the bodies change their state
     * by update_SliderJoint().\n\n More information can be found in: \cite Deul2014
     *
     ...
     */
    static bool
    update_TargetPositionMotorSliderJoint( const Vector3r& x0,    // center of mass of body 0
                                           const Quaternionr& q0, // rotation of body 0
                                           const Vector3r& x1,    // center of mass of body 1
                                           const Quaternionr& q1, // rotation of body 1
                                           Eigen::Matrix<Real, 3, 14>& jointInfo );
     ...
     * A motor slider joint removes two translational and three rotational degrees of freedom
     * between the bodies. Moreover, a target position can be enforced on the remaining translation
     * axis. The motor slider joint info must be generated in the initialization process of the
     * model by calling the function init_TargetPositionMotorSliderJoint() and updated each time the
     * bodies change their state by update_TargetPositionMotorSliderJoint().\n\n More information
     * can be found in: \cite Deul2014
     *
     ...
     * information must be generated in the beginning by calling
     * init_TargetPositionMotorSliderJoint() and updated each time the bodies change their state by
     ...
     */
    static bool
    update_TargetVelocityMotorSliderJoint( const Vector3r& x0,    // center of mass of body 0
                                           const Quaternionr& q0, // rotation of body 0
                                           const Vector3r& x1,    // center of mass of body 1
                                           const Quaternionr& q1, // rotation of body 1
                                           Eigen::Matrix<Real, 3, 14>& jointInfo );
    ...
    /** Perform a solver step for a motor slider joint which links two rigid bodies.
     * A motor slider joint removes two translational and three rotational degrees of freedom
     * between the bodies. Moreover, a target velocity can be enforced on the remaining translation
     * axis. The motor slider joint info must be generated in the initialization process of the
     * model by calling the function init_TargetVelocityMotorSliderJoint() and updated each time the
     * bodies change their state by update_TargetVelocityMotorSliderJoint().\n\n More information
     * can be found in: \cite Deul2014
     *
     * \image html motorsliderjoint.jpg "motor slider joint"
     * \image latex motorsliderjoint.jpg "motor slider joint" width=0.5\textwidth
     ...
     * @param jointInfo Motor slider joint information which is required by the solver. This
     * information must be generated in the beginning by calling
     * init_TargetVelocityMotorSliderJoint() and updated each time the bodies change their state by
    ...
    static bool velocitySolve_TargetVelocityMotorSliderJoint(
        ...
        Vector3r& corr_v0, Vector3r& corr_omega0, Vector3r& corr_v1, Vector3r& corr_omega1 );
     ...
     */
    static bool
    init_TargetAngleMotorHingeJoint( const Vector3r& x0,                 // center of mass of body 0
                                     const Quaternionr& q0,              // rotation of body 0
                                     const Vector3r& x1,                 // center of mass of body 1
                                     const Quaternionr& q1,              // rotation of body 1
                                     const Vector3r& hingeJointPosition, // position of hinge joint
     ...
     * A motor hinge joint removes three translational and two rotational degrees of freedom between
     * the bodies. Moreover, a target angle can be enforced on the remaining rotation axis. The
     * motor hinge joint info must be generated in the initialization process of the model by
     * calling the function init_TargetAngleMotorHingeJoint() and updated each time the bodies
     ...
     */
    static bool
    update_TargetVelocityMotorHingeJoint( const Vector3r& x0,    // center of mass of body 0
                                          const Quaternionr& q0, // rotation of body 0
                                          const Vector3r& x1,    // center of mass of body 1
                                          const Quaternionr& q1, // rotation of body 1
                                          Eigen::Matrix<Real, 3, 14>& jointInfo );
    ...
    /** Perform a solver step for a motor hinge joint which links two rigid bodies.
     * A motor hinge joint removes three translational and two rotational degrees of freedom between
     * the bodies. Moreover, a target velocity can be enforced on the remaining rotation axis by
     ...
     * @param jointInfo Motor hinge joint information which is required by the solver. This
     * information must be generated in the beginning by calling
     * init_TargetVelocityMotorHingeJoint() and updated each time the bodies change their state by
     * update_TargetVelocityMotorHingeJoint().
     * @param corr_x0 position correction of center of mass of first body
     * @param corr_q0 rotation correction of first body
     * @param corr_x1 position correction of center of mass of second body
     * @param corr_q1 rotation correction of second body
     ...
     * A motor hinge joint removes three translational and two rotational degrees of freedom between
     * the bodies. Moreover, a target velocity can be enforced on the remaining rotation axis. The
     * motor hinge joint info must be generated in the initialization process of the model by
     * calling the function init_TargetVelocityMotorHingeJoint() and updated each time the bodies
     * change their state by update_TargetVelocityMotorHingeJoint().\n\n
     * More information can be found in: \cite Deul2014
     *
     * \image html motorhingejoint.jpg "motor hinge joint"
     ...
     * @param jointInfo Stores the local and global position of the connector point in the rigid
     * body. The info must be updated in each simulation step by calling
     * update_RigidBodyParticleBallJoint().\n The joint info contains the following columns:\n 0:
     * connector in rigid body (local)\n 1:	connector in rigid body (global)\n
     */
    static bool
    init_RigidBodyParticleBallJoint( const Vector3r& x0,    // center of mass of the rigid body
     ...
     */
    static bool
    update_RigidBodyParticleBallJoint( const Vector3r& x0,    // center of mass of the rigid body
                                       const Quaternionr& q0, // rotation of the rigid body body
                                       const Vector3r& x1,    // position of the particle
                                       Eigen::Matrix<Real, 3, 2>& jointInfo );
     ...
     * The joint info must be generated in the initialization process of the model
     * by calling the function init_RigidBodyParticleBallJoint() and updated each time the rigid
     * body changes its state by update_RigidBodyParticleBallJoint().\n\n More information can be
     * found in: \cite Deul2014
     *
     ...
     * and updated each time the rigid body changes its state by
     * update_RigidBodyParticleBallJoint().
    ...
    static bool velocitySolve_ParticleRigidBodyContactConstraint(
        ...
        Real& sum_impulses,                        // sum of all impulses
        Eigen::Matrix<Real, 3, 5>& constraintInfo, // precomputed contact info
        Vector3r& corr_v0, Vector3r& corr_v1, Vector3r& corr_omega1 );
};
} // namespace PBD
