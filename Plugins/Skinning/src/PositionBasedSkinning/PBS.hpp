...
private:
    ...
    /* Weight Particles */
    Scalar distancePointSegment (const Ra::Core::Vector3 &point, const Ra::Core::Vector3 &startSegment, const Ra::Core::Vector3 &endSegment);
    void searchNearestBone (const  Ra::Core::Vector3 &particle, Ra::Core::Vector3 &startBone, Ra::Core::Vector3 &endBone,
                                 unsigned int &indexNearestBone, Scalar &distanceNearestBone);

    void smoothWeights (const Ra::Core::Graph::AdjacencyList &skeletonGraph, const unsigned int indexParticle,
                             const Ra::Core::Vector3 &particle, const unsigned int indexNearestBone,
                             const Ra::Core::Vector3 &startBone, const Ra::Core::Vector3 &endBone);

    void weightParticle(const Ra::Core::Graph::AdjacencyList &skeletonGraph, const unsigned int indexParticle, const Ra::Core::Vector3 &particle,
                        const unsigned int indexNearestBone, const Ra::Core::Vector3 &startBone, const Ra::Core::Vector3 &endBone);
    ...
    void linearBlendSkinning(const Ra::Core::Animation::Pose &pose, PBD::ParticleData &pd);

    /* Init Surface Mesh */
    Scalar computeDeterminant(const Ra::Core::Vector3 &vertex0, const Ra::Core::Vector3 &vertex1, const Ra::Core::Vector3 &vertex2, const Ra::Core::Vector3 &vertex3);

    void getDeterminants (const Ra::Core::Vector3 &tetVertex0, const Ra::Core::Vector3 &tetVertex1,
                           const Ra::Core::Vector3 &tetVertex2, const Ra::Core::Vector3 &tetVertex3, const Ra::Core::Vector3 &surfaceVertex,
                           Scalar &determinant0, Scalar &determinant1, Scalar &determinant2, Scalar &determinant3, Scalar &determinant4);

    const Ra::Core::Vector4 getBarycentricCoordinate(const Scalar determinant0, const Scalar determinant1,
                                                 const Scalar determinant2, const Scalar determinant3, const Scalar determinant4);

    bool checkPointInTetra(Scalar determinant0, Scalar determinant1,
    ...
    Scalar getSquaredDistancePointToTetrahedron(const Ra::Core::Vector3 &tetVertex0, const Ra::Core::Vector3 &tetVertex1,
                                                      const Ra::Core::Vector3 &tetVertex2, const Ra::Core::Vector3 &tetVertex3, const Ra::Core::Vector3 &surfaceVertex);

    // Find the barycentric coordinate for a single vertex in a list of tetrahedra
    void findBarycentricCoordinateTetrahedronForVertex(const Ra::Core::Vector3 &surfaceVertex, const PBD::TetModel::Tets &tetras, const PBD::ParticleData &pd);
