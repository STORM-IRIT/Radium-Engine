...
namespace SkinningPlugin{
	...
	SkinningWidget::SkinningWidget( QWidget * parent ) : QFrame( parent ),
		...
		m_skinningSelect->insertItems( 0, QStringList()
            << "Linear Blend Skinning" << "Dual Quaternion Skinning" << "Center of Rotation skinning" << "Position Based Skinning");
        ...
        m_actionPBS = new QAction(QIcon(":/Assets/Images/PBS.png"), QString("Position Based Skinning"),nullptr);
    ...
    QWidget * SkinningWidget::createPBSInterface () {
        ...
        connect(volumetricMeshPathSelect, SIGNAL(released()), this, SLOT(chooseVolumetricMeshPath()));
    ...
    void SkinningWidget::createPBStiffnesses () {
        ...
        // Signal connection
        connect(m_stretchStiffnessSelect, SIGNAL(valueChanged(double)),
                this, SLOT(onStretchStiffnessSelectValueChanged(double)));

        connect(m_compressionStiffnessSelect, SIGNAL(valueChanged(double)),
                this, SLOT(onCompressionSelectValueChanged(double)));

        connect(m_negVolumeStiffnessSelect, SIGNAL(valueChanged(double)),
                this, SLOT(onNegVolumeSelectValueChanged(double)));

        connect(m_posVolumeStiffnessSelect, SIGNAL(valueChanged(double)),
                this, SLOT(onPosVolumeStiffnessSelectValueChanged(double)));
    ...
    void SkinningWidget::chooseVolumetricMeshPath () {
        QString fileName = QFileDialog::getOpenFileName(this,tr("Choose the Corresponding Volumetric Mesh File"),
                                                        "", tr("Mesh Files (*.mesh)"));
        PositionBasedSkinning::PBS &pbs = m_current->getPBS();
        ...
        if (!pbs.isVolumetricMeshValid())
        {
            m_pathVolumetricMeshValidIndication->setText(tr("The selected Volumetric Mesh File is not Valid"));
        }
        else
        {
