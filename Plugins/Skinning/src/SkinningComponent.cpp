...
void SkinningComponent::setupSkinning() {
    ...
    if ( hasSkel && hasWeights && hasMesh && hasRefPose )
    {
        ...
        m_verticesWriter = compMsg->rwCallback<Ra::Core::Container::Vector3Array>(
            getEntity(), m_contentsName + "v" );
        m_normalsWriter = compMsg->rwCallback<Ra::Core::Container::Vector3Array>(
            getEntity(), m_contentsName + "n" );
        m_duplicateTableGetter = compMsg->getterCallback<std::vector<Ra::Core::Container::Index>>(
        ...
        // and mesh.
        ON_ASSERT( bool skinnable = compMsg->canSet<TriangleMesh>( getEntity(), m_contentsName ) );
        CORE_ASSERT(
            skinnable,
