...
public:
  ...
  std::shared_ptr<Ra::Engine::Rendering::RenderObject> getSelected() { 
      return m_selectedRo; 
  }

  void resetSelected() { m_selectedRo = nullptr; m_currentPoint = -1; }
...
private:
  ...
  inline float distanceSquared(const Ra::Core::Vector3f& pointA, const Ra::Core::Vector3f& pointB);
  ...
  void updateCurve( unsigned int curveId, unsigned int curveIdSize, 
  ...
  void subdivisionBezier( int vertexIndex, unsigned int curveIndex, const Ra::Core::Vector3Array& ctrlPts );
  void refreshPoint( unsigned int pointIndex, const Ra::Core::Vector3& newPoint = Ra::Core::Vector3::Zero()); 
