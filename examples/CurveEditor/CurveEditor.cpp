...
CurveEditor::CurveEditor( const Ra::Core::VectorArray<Ra::Core::Geometry::Curve2D::Vector>& polyline, MyViewer* viewer )
  : Ra::Engine::Scene::Entity( "Curve Editor" ),
  m_viewer( viewer )
{
    ...
    auto e = PointFactory::createPointComponent( this, allCtrlPts[0][0], {0}, namePt, Ra::Core::Utils::Color::Blue() );
    ...
    for (unsigned int i = 0; i < allCtrlPts.size(); i++) {
        ...
        if( i == allCtrlPts.size()-1 )
            e = PointFactory::createPointComponent( this, allCtrlPts[i][3], {i}, namePt, Ra::Core::Utils::Color::Blue() );
        else
            e = PointFactory::createPointComponent( this, allCtrlPts[i][3], {i, i+1}, namePt, Ra::Core::Utils::Color::Blue() );
        m_pointEntities.push_back( e );
        nameIndex++;
    }
...
unsigned int CurveEditor::getPointIndex( unsigned int curveIdSize, 
    ...
    if( curveIdSize > 1 || currentPtIndex == m_pointEntities.size()-1){
        ...
        pointIndex = (currentPtIndex%3);
    }
    return pointIndex;
}

void CurveEditor::updateCurve( unsigned int curveId, unsigned int curveIdSize, 
...
void CurveEditor::refreshPoint( unsigned int pointIndex, const Ra::Core::Vector3& newPoint) {
    ...
    auto pointEntity = PointFactory::createPointComponent( this, point, pointCurve, pointName, pointColor );
...
inline float CurveEditor::distanceSquared(const Ra::Core::Vector3f& pointA, const Ra::Core::Vector3f& pointB){
...
void CurveEditor::subdivisionBezier( int vertexIndex, unsigned int curveIndex, 
                                    const Ra::Core::Vector3Array& ctrlPts ) {
    auto bezier = Ra::Core::Geometry::CubicBezier( 
                              Ra::Core::Vector2(ctrlPts[0].x(), ctrlPts[0].z()), 
    ...
    auto ptE = PointFactory::createPointComponent(this, clickedPoint, {curveIndex, curveIndex+1}, namePt, Ra::Core::Utils::Color::Blue());
...
void CurveEditor::addPointAtEnd( const Ra::Core::Vector3& worldPos ) {
    ...
    auto ptC = PointFactory::createPointComponent( this, ctrlPts[1], { (pointIndex/3) }, namePt );
    m_pointEntities.push_back( ptC );
    namePt = "CtrlPt_" + std::to_string( pointIndex+1 );
    auto eb = PointFactory::createPointComponent( this, ctrlPts[2], { (pointIndex/3) }, namePt );
    m_pointEntities.push_back( eb );
    namePt = "CtrlPt_" + std::to_string( pointIndex+2 );
    ptC = PointFactory::createPointComponent( this, ctrlPts[3], { (pointIndex/3) }, namePt, Ra::Core::Utils::Color::Blue() );
    m_pointEntities.push_back( ptC );
}
...
void CurveEditor::addPointInCurve( const Ra::Core::Vector3& worldPos, int mouseX, int mouseY ) {
    ...
    for(int i = mouseX - radius; i < mouseX + radius && !found; i++){
        for(int j = mouseY - radius; j < mouseY + radius; j++){
            ...
            Ra::Engine::Data::ViewingParameters renderData{ camera->getViewMatrix(),
            ...
            if(pres.getRoIdx().isValid() && m_roMgr->exists(pres.getRoIdx())) { 
                ...
                if(mesh != nullptr){
                    ...
                    for(unsigned int w = 0; w < mesh->getNumVertices(); w++){
                        auto dist = distanceSquared(worldPos, mesh->getCoreGeometry().vertices()[w]);
                        if(dist < bestV ){
                            bestV = dist;
                            vIndex = w;
                        }
...
bool CurveEditor::processHover( std::shared_ptr<Ra::Engine::Rendering::RenderObject> ro ) {
    ...
    ro->getMaterial()->getParameters().addParameter( "material.color", Ra::Core::Utils::Color::Red() );
    m_selectedRo = ro;
    return true;
}
...
void CurveEditor::processUnhovering() {
    ...
    m_selectedRo->getMaterial()->getParameters().addParameter( "material.color", pointCmp->m_color );
...
void CurveEditor::processPicking( const Ra::Core::Vector3& worldPos ) {
    ...
    if( m_smooth &&
        !( m_currentPoint == 0 || m_currentPoint == 1 ||
        ...
        if( m_currentPoint % 3 == 2 ) {
            ...
            auto symTransform = computePointTransform(pointComponent, pointMid->m_point, worldPos);
            ...
            if( m_tangentPoints.empty() ) {
                m_tangentPoints.push_back( m_currentPoint+2 );
            }
        }
            ...
            auto leftRo = m_roMgr->getRenderObject(m_pointEntities[m_currentPoint-1]->getRenderObjects()[0]);
            auto leftTransform = leftRo->getTransform() * transformTranslate;
            leftRo->setLocalTransform( leftTransform );
            auto rightRo = m_roMgr->getRenderObject(m_pointEntities[m_currentPoint+1]->getRenderObjects()[0]);
