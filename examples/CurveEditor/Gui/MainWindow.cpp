...
MainWindow::MainWindow(QWidget *parent) : MainWindowInterface(parent) {
    ...
    m_sceneModel = new Ra::Gui::ItemModel(
        Ra::Engine::RadiumEngine::getInstance(), this);
    m_selectionManager =
        new Ra::Gui::SelectionManager(m_sceneModel, this);
...
MyViewer *MainWindow::getViewer() { return m_viewer; }
...
void MainWindow::cleanup() { m_viewer = nullptr; }

void MainWindow::createConnections() {
    connect( getViewer(),
             &MyViewer::toggleBrushPicking,
             this,
             &MainWindow::toggleCirclePicking );
    ...
    connect( m_editCurveButton, &QPushButton::pressed, this, &MainWindow::onEditStrokeButtonPressed );
    connect( m_hideStrokeButton, &QPushButton::clicked, this, &MainWindow::onHideStrokeButtonClicked );
    ...
    connect( getViewer(), &MyViewer::onMouseDoubleClick, this, &MainWindow::handleMouseDoubleClickEvent );
}
...
void MainWindow::onHideStrokeButtonClicked(){
    ...
    if( m_hideStrokeButton->isChecked() ){
        ro->setVisible( false );
    }
    else ro->setVisible( true );
    m_viewer->needUpdate();
}

void MainWindow::onEditStrokeButtonPressed() {
    if( m_polyline.empty() || m_edited )
        return;
    m_edited = true;
